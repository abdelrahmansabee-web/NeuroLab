...
KIN_PDF_VARS = [
    (re.compile(r"NVP|Number of Velocity Peaks", re.I), "nvp"),
    (re.compile(r"Straightness", re.I), "straightness"),
    (re.compile(r"Pause Time", re.I), "pause_time_sec"),
    (re.compile(r"Number of Stops", re.I), "number_of_stops"),
    (re.compile(r"SPARC", re.I), "sparc"),
    (re.compile(r"Trunk\s*Ratio|Trunk/Palm", re.I), "trunk_ratio"),
    (re.compile(r"Shoulder Vert|Shoulder elevation", re.I), "shoulder_elevation_cm"),
    (re.compile(r"Peak (?:hand )?velocity|Peak Velocity", re.I), "peak_velocity_cm_s"),
    (re.compile(r"Elbow angle", re.I), "elbow_angle_mean_deg"),
    (re.compile(r"Shoulder flexion \(mean\)|Mean shoulder flexion", re.I), "shoulder_flexion_mean_deg"),
    (re.compile(r"Peak shoulder flexion|Peak shoulder flex", re.I), "peak_shoulder_flexion_vel_deg_s"),
    (re.compile(r"Movement time|Duration", re.I), "movement_time_sec"),
    (re.compile(r"Peak elbow angular velocity", re.I), "peak_elbow_ang_vel_deg_s"),
]
...
def normalize_restored_patient(patient: Dict[str, Any]) -> Dict[str, Any]:
    """Map PDF kinematics aliases into analysisResults and infer pre/post flags."""
    if not isinstance(patient, dict):
        return {}
    for key in SECTION_KEYS:
        if not isinstance(patient.get(key), dict):
            patient[key] = {}
    ...
    for src, phase in (
        ...
        ("result_healthy", "baseline"),
    ):
        block = kin.get(src)
        if isinstance(block, dict) and block:
            analysis[phase] = {**(analysis.get(phase) or {}), **block}
            if phase == "baseline":
                kin.setdefault("status_baseline", "completed")
            else:
                kin.setdefault(f"status_{phase}", "completed")
    if analysis:
        kin["analysisResults"] = analysis
    patient["kinematics"] = kin

    vas = patient.get("vas") if isinstance(patient.get("vas"), dict) else {}
    rest = vas.get("rest") if isinstance(vas.get("rest"), dict) else {}
    motor = patient.get("motorchange") if isinstance(patient.get("motorchange"), dict) else {}
    vams = patient.get("vams") if isinstance(patient.get("vams"), dict) else {}
    happy = vams.get("happy") if isinstance(vams.get("happy"), dict) else {}
    ar = kin.get("analysisResults") if isinstance(kin.get("analysisResults"), dict) else {}
    patient["_hasPre"] = bool(
        patient.get("_hasPre")
        or rest.get("pre")
        or motor.get("control")
        or happy.get("pre")
        or (isinstance(ar.get("pre"), dict) and ar["pre"])
        or (isinstance(kin.get("result_pre"), dict) and kin["result_pre"])
    )
    patient["_hasPost"] = bool(
        patient.get("_hasPost")
        or rest.get("post")
        or motor.get("difference")
        or happy.get("post")
        or (isinstance(ar.get("post"), dict) and ar["post"])
        or (isinstance(kin.get("result_post"), dict) and kin["result_post"])
    )
    return patient
...
def parse_clinical_report_pdf(text: str) -> Dict[str, Any]:
    ...
    if date_m:
        ...
        if name_m:
            # Keep full display name (folder restore already has Study ID).
            patient["demographics"]["name"] = re.sub(r"\s+", " ", name_m.group(1).strip())
    ...
    return normalize_restored_patient(patient)
