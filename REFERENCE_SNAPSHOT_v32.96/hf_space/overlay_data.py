...
def _fill_edge_gaps(edge_y: np.ndarray) -> np.ndarray:
    y = np.asarray(edge_y, dtype=float)
...
def _pack_table_edge_profile(edge_y_norm: np.ndarray, n: int = 96) -> List[Optional[float]]:
    w = len(edge_y_norm)
    if w < 2:
        return []
    ...
    for i in range(n):
        ...
        if np.isfinite(a) and np.isfinite(b):
            out.append(round(float(a + (b - a) * f), 4))
        elif np.isfinite(a):
...
def _arm_support_y(*ys: Optional[float]) -> Optional[float]:
    """Lower (larger y) of the resting hand/wrist — that's the table surface."""
    vals = [float(y) for y in ys if y is not None and np.isfinite(y) and float(y) >= 0.45]
    if not vals:
        vals = [float(y) for y in ys if y is not None and np.isfinite(y)]
    return max(vals) if vals else None


def _clamp_table_x(
    x: float,
    *pts: Optional[Tuple[float, float]],
) -> float:
    xs = [float(p[0]) for p in pts if p is not None and np.isfinite(p[0])]
    if not xs:
        return float(x)
...
def _hough_table_y_in_band(disp: np.ndarray, y_lo: float, y_hi: float) -> Optional[float]:
    """Long horizontal edge inside a band around the resting arm (the table)."""
    h, w = disp.shape[:2]
    y0 = int(np.clip(y_lo * h, 0, h - 2))
    y1 = int(np.clip(y_hi * h, y0 + 2, h))
    gray = cv2.cvtColor(disp, cv2.COLOR_BGR2GRAY)
    roi = cv2.GaussianBlur(gray[y0:y1], (5, 5), 0)
    ...
    for line in lines:
        ...
        if score > best_score:
            best_score = score
            best_y = y / h
    return float(best_y) if best_y is not None else None


def _is_seated_table_y(y: Optional[float]) -> bool:
    return y is not None and np.isfinite(y) and 0.56 <= float(y) <= 0.90


def _is_clinic_table_cream(r: np.ndarray, g: np.ndarray, b: np.ndarray) -> np.ndarray:
    lum = 0.299 * r + 0.587 * g + 0.114 * b
    chroma = np.maximum(np.maximum(r, g), b) - np.minimum(np.minimum(r, g), b)
    rb = r - b
    return (lum > 145) & (lum < 215) & (rb > 22) & (rb < 95) & (chroma < 95) & (r >= g - 8)


def _cream_table_surface_y(
    disp: np.ndarray,
    ...
    if h < 24 or w < 24:
        return None
    bb = disp[:, :, 0].astype(np.float32)
    gg = disp[:, :, 1].astype(np.float32)
    rr = disp[:, :, 2].astype(np.float32)
    cream = _is_clinic_table_cream(rr, gg, bb)
    min_y = 0.56
    if shoulder_y is not None and np.isfinite(shoulder_y):
        min_y = max(min_y, float(shoulder_y) + 0.10)
    y0 = int(h * 0.42)
    y1 = int(h * 0.90)
    step = max(1, h // 80)
    rows = []
    for y in range(y0, min(h, y1 + 1), step):
        frac = float(np.mean(cream[y]))
        rows.append((y / float(h), frac))
    thresh = 0.55
    bands = []
    start = None
    last_y = None
    for y_norm, frac in rows:
        last_y = y_norm
        if frac >= thresh:
            if start is None:
                start = y_norm
        elif start is not None:
            bands.append((start, y_norm))
            start = None
    if start is not None and last_y is not None:
        bands.append((start, max(last_y, start + 0.02)))
    lower = [
        b for b in bands
        if (b[1] - b[0]) >= 0.035 and b[0] <= 0.90 and b[0] >= min_y - 0.03
    ]
    y = lower[-1][0] if lower else None
    if y is None:
        saw_high = False
        in_dip = False
        for y_norm, frac in rows:
            if y_norm < 0.48 or y_norm > 0.82:
                continue
            if not saw_high and frac >= 0.65:
                saw_high = True
                continue
            if saw_high and (not in_dip) and frac <= 0.52:
                in_dip = True
                continue
            if in_dip and frac >= 0.60 and y_norm >= min_y - 0.02:
                y = y_norm
                break
    if not _is_seated_table_y(y):
        return None
    y = float(max(y, min_y))
    return float(min(0.90, y + 0.02))


def _palm_color_table_mask(
    hsv: np.ndarray,
    palm_xy: Tuple[float, float],
    shoulder_xy: Optional[Tuple[float, float]] = None,
) -> Optional[np.ndarray]:
    """Grow table color from a patch just below the resting palm (wood or teal)."""
    h, w = hsv.shape[:2]
    px = int(np.clip(float(palm_xy[0]) * w, 2, w - 3))
    py = int(np.clip(float(palm_xy[1]) * h, 2, h - 3))
    samples = []
    for dx, dy in ((0, 10), (12, 12), (-12, 12), (0, 20), (18, 8), (-18, 8)):
        x = int(np.clip(px + dx, 2, w - 3))
        y = int(np.clip(py + dy, 2, h - 3))
        samples.append(hsv[y - 2 : y + 3, x - 2 : x + 3].reshape(-1, 3))
    if not samples:
        return None
    med = np.median(np.concatenate(samples, axis=0), axis=0)
    lo = np.array([
        max(0, med[0] - 16),
        max(0, med[1] - 60),
        max(0, med[2] - 60),
    ...
    y_cut = int(h * 0.40)
    if shoulder_xy is not None and np.isfinite(shoulder_xy[1]):
        y_cut = max(y_cut, int(float(shoulder_xy[1]) * h + 0.06 * h))
    m[:y_cut, :] = 0
    return m


def _table_mask_from_frame(
    disp: np.ndarray,
    palm_xy: Optional[Tuple[float, float]] = None,
    shoulder_xy: Optional[Tuple[float, float]] = None,
) -> Optional[np.ndarray]:
    """Clinic table mask: teal cloth, light wood, and color sampled at the rest palm."""
    h, w = disp.shape[:2]
    hsv = cv2.cvtColor(disp, cv2.COLOR_BGR2HSV)
    teal = cv2.inRange(hsv, np.array([78, 22, 30]), np.array([148, 255, 255]))
    wood = cv2.inRange(hsv, np.array([8, 18, 70]), np.array([40, 140, 245]))
    y0 = int(h * 0.38)
    if shoulder_xy is not None and np.isfinite(shoulder_xy[1]):
        y0 = max(y0, int(float(shoulder_xy[1]) * h + 0.04 * h))
    wood[:y0, :] = 0
    if shoulder_xy is not None and np.isfinite(shoulder_xy[1]):
        teal[: max(0, int(float(shoulder_xy[1]) * h + 0.04 * h)), :] = 0
    mask = cv2.bitwise_or(teal, wood)
    if palm_xy is not None and np.isfinite(palm_xy[0]) and np.isfinite(palm_xy[1]):
        palm_mask = _palm_color_table_mask(hsv, palm_xy, shoulder_xy)
    ...
    def _score(c: np.ndarray) -> float:
        ...
        return float(cv2.contourArea(c)) * (1.0 + 0.85 * (cy / max(h, 1)))

    cnt = max(candidates, key=_score)
    cmask = np.zeros((h, w), dtype=np.uint8)
    cv2.drawContours(cmask, [cnt], -1, 255, -1)
    return cmask


def _upper_edge_from_mask(mask: np.ndarray) -> np.ndarray:
    """First table pixel scanning down each column (far/back edge of a tall blob)."""
    hits = mask > 0
    first = np.argmax(hits, axis=0).astype(float)
    edge = np.where(hits.any(axis=0), first, np.nan)
    return edge


def _scan_down_edge(mask: np.ndarray, y0_px: float) -> np.ndarray:
    """Table surface under a height: first table pixel at or below y0 in each column."""
    h, w = mask.shape[:2]
    y0 = int(np.clip(y0_px, 0, h - 1))
    hits = mask[y0:, :] > 0
    first = np.argmax(hits, axis=0).astype(float)
    return np.where(hits.any(axis=0), y0 + first, np.nan)


def _snap_table_y_to_rest_arm(
    table_y: Optional[float],
    palm_y: Optional[float],
    shoulder_y: Optional[float] = None,
) -> Optional[float]:
    """Resting hand is on the table — reject chest-high or floor-low edges."""
    if palm_y is None or not np.isfinite(palm_y):
        return float(table_y) if table_y is not None and np.isfinite(table_y) else None
    palm = float(palm_y)
    if table_y is None or not np.isfinite(table_y):
        return palm
    ty = float(table_y)
    if ty < palm - 0.06:
        return palm
    if shoulder_y is not None and np.isfinite(shoulder_y) and ty < float(shoulder_y) + 0.10:
        return palm
    if ty > palm + 0.12:
        return palm
    return ty
...
def _nearest_table_xy(
    ...
    """Nearest table pixel to the shoulder, biased to sit under it (small dx)."""
    ys, xs = np.where(mask > 0)
    if len(xs) < 30:
        return None
    below = ys >= min(h - 2, int(sy + 0.06 * h))
    xs, ys = xs[below], ys[below]
    if len(xs) < 20:
        return None
    if np.isfinite(palm_y):
        py = float(palm_y) * h
        near = np.abs(ys - py) <= 0.14 * h
        if int(np.count_nonzero(near)) >= 20:
            xs, ys = xs[near], ys[near]
    dx = (xs - sx) / max(float(w), 1.0)
    dy = (ys - sy) / max(float(h), 1.0)
    cost = (dx * 2.4) ** 2 + dy ** 2
...
def _read_display_frame(
    ...
    idx = int(np.clip(round(float(t_sec) * fps), 0, max(0, n - 1)))
    cap.set(cv2.CAP_PROP_POS_FRAMES, idx)
    ret, frame = cap.read()
    cap.release()
    if not ret or frame is None:
        return None
    return _frame_to_display(frame, orientation)
...
def _detect_cup(
    ...
    """Paper cup sitting on the table: patterned cream cylinder; base is the surface."""
    if disp is None or disp.size == 0:
        return None
    h, w = disp.shape[:2]
    if h < 24 or w < 24:
        return None
    bgr = disp
    y0w, y1w = int(h * 0.02), max(int(h * 0.02) + 1, int(h * 0.16))
    x0w, x1w = int(w * 0.18), int(w * 0.82)
    wall = np.median(bgr[y0w:y1w, x0w:x1w].reshape(-1, 3), axis=0).astype(np.float32)
    b = bgr[:, :, 0].astype(np.int16)
    g = bgr[:, :, 1].astype(np.int16)
    r = bgr[:, :, 2].astype(np.int16)
    lum = (0.114 * b + 0.587 * g + 0.299 * r).astype(np.float32)
    dwall = (
        np.abs(b.astype(np.float32) - wall[0])
        + np.abs(g.astype(np.float32) - wall[1])
        + np.abs(r.astype(np.float32) - wall[2])
    )
    chroma = np.maximum(np.maximum(r, g), b) - np.minimum(np.minimum(r, g), b)
    is_print = (b > r + 6) & (b >= g - 10) & (lum > 48) & (lum < 205)
    is_cream = (lum >= 172) & ((r - b) < 52) & (chroma < 78) & (dwall > 16)
    ok = is_print | is_cream
    ok[: int(h * 0.26), :] = False
    ok[int(h * 0.975) :, :] = False
    ...
    if num <= 1:
        return None
    px = float(palm_xy[0]) if palm_xy is not None and np.isfinite(palm_xy[0]) else None
    py = float(palm_xy[1]) if palm_xy is not None and np.isfinite(palm_xy[1]) else None
    best = None
    best_score = -1.0
    for i in range(1, num):
        ...
        if area < 40:
            continue
        wn, hn = bw / float(w), bh / float(h)
        if wn < 0.028 or wn > 0.24:
            continue
        if hn < 0.065 or hn > 0.44:
            continue
        aspect = bh / max(float(bw), 1.0)
        if aspect < 1.2 or aspect > 6.8:
            continue
        y_bottom = (y + bh) / float(h)
        y_top = y / float(h)
        if y_bottom < 0.50 or y_bottom > 0.985:
            continue
        if y_top < 0.16 or y_top > 0.84:
            continue
        roi_print = is_print[y : y + bh, x : x + bw]
        roi_cream = is_cream[y : y + bh, x : x + bw]
        print_frac = float(np.count_nonzero(roi_print)) / float(max(roi_print.size, 1))
        cream_frac = float(np.count_nonzero(roi_cream)) / float(max(roi_cream.size, 1))
        if cream_frac < 0.18 and print_frac < 0.04:
            continue
        var = float(np.std(lum[y : y + bh, x : x + bw]))
        if print_frac < 0.015 and var < 10 and cream_frac < 0.40:
            continue
        score = aspect * 6.0 + print_frac * 90.0 + cream_frac * 24.0 + var * 0.35
        y_below = min(h - 1, y + bh + max(2, int(h * 0.012)))
        if y_below > y + bh - 1:
            body_l = float(np.mean(lum[y + bh // 2, x : x + bw]))
            below_l = float(np.mean(lum[y_below, x : x + bw]))
            if below_l < body_l - 6:
                score += 28.0
        cx = (x + bw * 0.5) / float(w)
        edge_bias = min(cx, 1.0 - cx)
        score += (0.38 - min(edge_bias, 0.38)) * 36.0
        if px is not None:
            dx = abs(cx - px)
            dy = abs(y_bottom - (py if py is not None else y_bottom))
            if dx < 0.48:
                score += (0.48 - dx) * 55.0
            if dy < 0.24:
                score += (0.24 - dy) * 42.0
        if score > best_score:
            ...
            best = {
                "x": round(float(cx), 4),
                "x0": round(float(x) / float(w), 4),
                "x1": round(float(x + bw) / float(w), 4),
                "y_top": round(float(y_top), 4),
                "y_base": round(float(y_bottom), 4),
            }
    return best


def _refine_table_under_shoulder(
    video_path: Optional[str],
    orientation: int,
    rest_shoulder: Tuple[float, float],
    rest_palm: Tuple[float, float],
    rest_t: float = 0.0,
    prior: Optional[Dict[str, Any]] = None,
    rest_wrist: Optional[Tuple[float, float]] = None,
    rest_elbow: Optional[Tuple[float, float]] = None,
    rest_trunk: Optional[Tuple[float, float]] = None,
) -> Optional[Dict[str, Any]]:
    """Table as a supporting plane: where the resting arm lies, not a chest edge.

    A person finds the table as the wide horizontal surface under the hand.
    Vision may refine Y by a few percent; it must not jump to the wall or chair.
    """
    sx_n, sy_n = float(rest_shoulder[0]), float(rest_shoulder[1])
    px_n, py_n = float(rest_palm[0]), float(rest_palm[1])
    wx_n = float(rest_wrist[0]) if rest_wrist is not None and np.isfinite(rest_wrist[0]) else px_n
    wy_n = float(rest_wrist[1]) if rest_wrist is not None and np.isfinite(rest_wrist[1]) else py_n
    support_y = _arm_support_y(py_n, wy_n)
    if support_y is None:
        support_y = py_n if np.isfinite(py_n) else None

    disp = _read_display_frame(video_path, orientation, rest_t) if video_path else None
    mask = None
    display_w = None
    display_h = None
    cup = None
    cream_y = None
    if disp is not None:
        display_h, display_w = disp.shape[:2]
        mask = _table_mask_from_frame(disp, palm_xy=(px_n, py_n), shoulder_xy=(sx_n, sy_n))
        cup = _detect_cup(disp, (px_n, py_n))
        cream_y = _cream_table_surface_y(disp, sy_n)
    elif prior is not None:
        mask = prior.get("mask")
        display_w = prior.get("display_w")
        display_h = prior.get("display_h")
        cup = prior.get("cup")

    cup_pt = None
    if cup is not None and cup.get("x") is not None and np.isfinite(cup.get("x")):
        cup_pt = (float(cup["x"]), float(cup.get("y_base") or 0.0))

    under_x = _clamp_table_x(
        sx_n, rest_palm, rest_wrist, rest_elbow, rest_trunk, rest_shoulder, cup_pt,
    )
    under_y = cream_y if _is_seated_table_y(cream_y) else support_y
    if not _is_seated_table_y(under_y):
        under_y = support_y if _is_seated_table_y(support_y) else cream_y

    hough_y = None
    if disp is not None and support_y is not None and not _is_seated_table_y(cream_y):
        hough_y = _hough_table_y_in_band(disp, support_y - 0.10, support_y + 0.10)
        if hough_y is not None and abs(hough_y - support_y) <= 0.05 and _is_seated_table_y(hough_y):
            under_y = hough_y

    if cup is not None and cup.get("y_base") is not None and np.isfinite(cup.get("y_base")):
        cy = float(cup["y_base"])
        if _is_seated_table_y(cy) and not _is_seated_table_y(cream_y):
            if support_y is None or abs(cy - float(support_y)) <= 0.22 or not _is_seated_table_y(support_y):
                under_y = cy

    edge_y_norm = None
    if mask is not None and display_w and display_h and support_y is not None:
        h, w = int(display_h), int(display_w)
        y0 = float(under_y if under_y is not None else support_y) * h - 0.04 * h
        edge_px = _scan_down_edge(mask, max(0.0, y0))
        edge_px = _fill_edge_gaps(edge_px)
        edge_y_norm = edge_px / float(h)
        nearest = _nearest_table_xy(mask, under_x * w, sy_n * h, px_n, support_y, h, w)
        if nearest is not None and abs(nearest[1] - support_y) <= 0.06:
            under_x = nearest[0]
            if cup is None and not _is_seated_table_y(cream_y) and _is_seated_table_y(nearest[1]):
                under_y = nearest[1]
        under_x = _clamp_table_x(
            under_x, rest_palm, rest_wrist, rest_elbow, rest_trunk, rest_shoulder, cup_pt,
        )
        ref_y = under_y if under_y is not None else support_y
        if edge_y_norm is not None:
            edge_y_norm = np.where(
                ~np.isfinite(edge_y_norm) | (np.abs(edge_y_norm - ref_y) > 0.08),
                ref_y,
                edge_y_norm,
            )
    else:
        if prior is not None and prior.get("edge_y") is not None:
            edge_y_norm = np.asarray(prior["edge_y"], dtype=float)
            display_w = prior.get("display_w") or display_w
            display_h = prior.get("display_h") or display_h

    if under_y is None or not np.isfinite(under_y):
        return prior
    if edge_y_norm is None and display_w:
        edge_y_norm = np.full(int(display_w), float(under_y), dtype=float)
    out = {
        "edge_y": edge_y_norm,
        "display_w": display_w if display_w is not None else (prior or {}).get("display_w"),
        "display_h": display_h if display_h is not None else (prior or {}).get("display_h"),
        "mask": mask if mask is not None else (prior or {}).get("mask"),
        "front_edge_y": float(under_y),
        "under_shoulder": {"x": round(float(under_x), 4), "y": round(float(under_y), 4)},
        "cup": cup,
        "cream_y": cream_y,
    }
    return out
...
def _hough_table_y_norm(disp: np.ndarray) -> Optional[float]:
    h, w = disp.shape[:2]
    gray = cv2.cvtColor(disp, cv2.COLOR_BGR2GRAY)
...
def _detect_table_front_edge(
    ...
    """Detect the clinic table surface in displayed (overlay) coordinates.

    Returns:
        - edge_y: normalized surface y per display column (upper table edge).
        - display_w, display_h
        - mask: last table mask
        - front_edge_y: median surface y
    """
    if cv2 is None:
        return None
    cap = cv2.VideoCapture(str(video_path))
    ...
    for i in indices:
        ...
        hy = _hough_table_y_norm(disp)
        if hy is not None:
            hough_ys.append(hy)
        mask = _table_mask_from_frame(disp)
        if mask is None:
            continue
        edge_y = _upper_edge_from_mask(mask)
        if not np.any(np.isfinite(edge_y)):
            continue
    ...
    if edge_arrays and last_mask is not None:
        edge_stack = np.stack(edge_arrays, axis=0)
        with np.errstate(invalid="ignore"):
            median_edge = np.nanmedian(edge_stack, axis=0)
        median_edge = _fill_edge_gaps(median_edge)
        edge_y_norm = median_edge / float(display_h)
    ...
    if hough_ys:
        hy = float(np.median(hough_ys))
        edge_y_norm = np.full(int(display_w), hy, dtype=float)
        return {
            "edge_y": edge_y_norm,
            "display_w": display_w,
            "display_h": display_h,
...
def _direct_hl_joint(
    ...
    *,
    ...
    Re-anchoring HL→pose wrist systematically floated tips off real fingers when
    pose wrist lagged the hand. Keep absolute HL coords; soft-hold short gaps.
    """
    reach = {
        "mcp": (0.001, 0.16),
        "ip": (0.001, 0.22),
    ...
    for i in range(n):
        ...
        if off_hand:
            ...
            prev_y = np.nan
            hold = 0
    out_x, out_y = smooth_xy_zero_phase(out_x, out_y)
    return out_x, out_y, conf
...
def _pose_finger_overlay_pair(
    ...
    vis = resample_visibility(_visibility(raw_df, lm_name), old_t, new_t)
    x, y = overlay_hide_unseen(x, y, vis)
    return x, y
    ...
    try:
        ...
        if side not in ("left", "right"):
            try:
                ...
                if float(det.get("confidence") or 0) < 0.2 and table_edge is not None and table_edge.get("mask") is not None:
        ...
        # Canonical time: frame index / native fps.
        # CSV `time` from int(1000/fps) runs slow on long PRE clips.
        native_fps = float(target_fs)
        if "fps" in df.columns:
            try:
                v = float(pd.to_numeric(df["fps"], errors="coerce").iloc[0])
                if v > 1:
                    native_fps = v
            except Exception:
                pass
        t = np.arange(len(df), dtype=float) / native_fps
        ...
        def _pair(name: str):
            ...
            vis = resample_visibility(_visibility(raw_df, name), t, new_t)
            x = _interpolate_small_gaps(x, max_gap=15)
            y = _interpolate_small_gaps(y, max_gap=15)
        ...
        # Early table-surface estimate: nearest table under the rest shoulder.
        rest_idx = int(onset_idx)
        rest_window = max(1, min(10, rest_idx + 1))
        rest_shoulder_x_early = float(np.nanmedian(shoulder_x[:rest_window])) if len(shoulder_x) else float("nan")
        rest_shoulder_y_early = float(np.nanmedian(shoulder_y[:rest_window])) if len(shoulder_y) else float("nan")
        rest_elbow_x_early = float(np.nanmedian(elbow_x[:rest_window])) if len(elbow_x) else float("nan")
        rest_elbow_y_early = float(np.nanmedian(elbow_y[:rest_window])) if len(elbow_y) else float("nan")
        rest_palm_x_early = float(np.nanmedian(palm_x[:rest_window])) if len(palm_x) else float("nan")
        rest_palm_y_early = float(np.nanmedian(palm_y[:rest_window])) if len(palm_y) else float("nan")
        rest_wrist_x_early = float(np.nanmedian(wrist_x[:rest_window])) if len(wrist_x) else float("nan")
        rest_wrist_y_early = float(np.nanmedian(wrist_y[:rest_window])) if len(wrist_y) else float("nan")
        rest_trunk_x_early = float(np.nanmedian(trunk_x[:rest_window])) if len(trunk_x) else float("nan")
        rest_trunk_y_early = float(np.nanmedian(trunk_y[:rest_window])) if len(trunk_y) else float("nan")
        arm_support_y = _arm_support_y(rest_palm_y_early, rest_wrist_y_early)
        table_under_shoulder = None
        overlay_cup = None
        if (
            resolved_video_path
            and resolved_video_path.exists()
            and np.isfinite(rest_shoulder_x_early)
            and np.isfinite(rest_shoulder_y_early)
        ):
            rest_t = float(new_t[min(rest_idx, len(new_t) - 1)]) if len(new_t) else 0.0
            palm_pair = (
                rest_palm_x_early if np.isfinite(rest_palm_x_early) else rest_shoulder_x_early,
                rest_palm_y_early if np.isfinite(rest_palm_y_early) else rest_shoulder_y_early + 0.35,
            )
            wrist_pair = (
                (rest_wrist_x_early, rest_wrist_y_early)
                if np.isfinite(rest_wrist_x_early) and np.isfinite(rest_wrist_y_early)
                else None
            )
            elbow_pair = (
                (rest_elbow_x_early, rest_elbow_y_early)
                if np.isfinite(rest_elbow_x_early) and np.isfinite(rest_elbow_y_early)
                else None
            )
            trunk_pair = (
                (rest_trunk_x_early, rest_trunk_y_early)
                if np.isfinite(rest_trunk_x_early) and np.isfinite(rest_trunk_y_early)
                else None
            )
            refined = _refine_table_under_shoulder(
                str(resolved_video_path),
                orientation,
                rest_shoulder=(rest_shoulder_x_early, rest_shoulder_y_early),
                rest_palm=palm_pair,
                rest_t=rest_t,
                prior=table_edge,
                rest_wrist=wrist_pair,
                rest_elbow=elbow_pair,
                rest_trunk=trunk_pair,
            )
            if refined is not None:
                table_edge = refined
                if refined.get("under_shoulder"):
                    table_under_shoulder = refined["under_shoulder"]
                if refined.get("cup"):
                    overlay_cup = refined["cup"]
        cream_y_early = None
        if table_edge and table_edge.get("cream_y") is not None:
            cream_y_early = table_edge.get("cream_y")
        table_surface_y_early = None
        if _is_seated_table_y(cream_y_early):
            table_surface_y_early = float(cream_y_early)
        elif table_under_shoulder and _is_seated_table_y(table_under_shoulder.get("y")):
            table_surface_y_early = float(table_under_shoulder["y"])
        cup_y = None
        if overlay_cup and overlay_cup.get("y_base") is not None and np.isfinite(overlay_cup.get("y_base")):
            cy = float(overlay_cup["y_base"])
            if _is_seated_table_y(cy) and not _is_seated_table_y(table_surface_y_early):
                if arm_support_y is None or abs(cy - float(arm_support_y)) <= 0.22 or not _is_seated_table_y(arm_support_y):
                    cup_y = cy
                    table_surface_y_early = cup_y
        if not _is_seated_table_y(table_surface_y_early) and _is_seated_table_y(arm_support_y):
            table_surface_y_early = float(arm_support_y)
        elif table_edge is not None and not _is_seated_table_y(table_surface_y_early):
            edge_y = np.asarray(table_edge["edge_y"], dtype=float)
            n_edge = len(edge_y)
            x_src = rest_shoulder_x_early if np.isfinite(rest_shoulder_x_early) else rest_elbow_x_early
            if n_edge >= 2 and np.isfinite(x_src):
                x_idx = int(np.clip(round(float(x_src) * (n_edge - 1)), 0, n_edge - 1))
                if np.isfinite(edge_y[x_idx]):
                    table_surface_y_early = float(edge_y[x_idx])
                elif np.any(np.isfinite(edge_y)):
                    table_surface_y_early = float(np.nanmedian(edge_y))
            elif np.any(np.isfinite(edge_y)):
                table_surface_y_early = float(np.nanmedian(edge_y))
        if cup_y is None and not _is_seated_table_y(cream_y_early):
            table_surface_y_early = _snap_table_y_to_rest_arm(
                table_surface_y_early,
                arm_support_y if arm_support_y is not None else rest_palm_y_early,
                rest_shoulder_y_early,
            )
            if not _is_seated_table_y(table_surface_y_early) and _is_seated_table_y(arm_support_y):
                table_surface_y_early = float(arm_support_y)
        cup_pt = None
        if overlay_cup and overlay_cup.get("x") is not None and np.isfinite(overlay_cup.get("x")):
            cup_pt = (float(overlay_cup["x"]), float(overlay_cup.get("y_base") or 0.0))
        if table_surface_y_early is not None and np.isfinite(rest_shoulder_x_early):
            tx = _clamp_table_x(
                float(table_under_shoulder["x"]) if table_under_shoulder and table_under_shoulder.get("x") is not None else rest_shoulder_x_early,
                (rest_palm_x_early, rest_palm_y_early),
                (rest_wrist_x_early, rest_wrist_y_early),
                (rest_elbow_x_early, rest_elbow_y_early),
                (rest_trunk_x_early, rest_trunk_y_early),
                (rest_shoulder_x_early, rest_shoulder_y_early),
                cup_pt,
            )
            table_under_shoulder = {
                "x": round(float(tx), 4),
                "y": round(float(table_surface_y_early), 4),
            }

        # Full skeleton + MediaPipe pose face mesh points (exact face bounds).
        nose_x, nose_y = _pair("NOSE")
        lear_x, lear_y = _pair("LEFT_EAR")
        rear_x, rear_y = _pair("RIGHT_EAR")
        leye_x, leye_y = _pair("LEFT_EYE")
        reye_x, reye_y = _pair("RIGHT_EYE")
        leye_i_x, leye_i_y = _pair("LEFT_EYE_INNER")
        leye_o_x, leye_o_y = _pair("LEFT_EYE_OUTER")
        reye_i_x, reye_i_y = _pair("RIGHT_EYE_INNER")
        reye_o_x, reye_o_y = _pair("RIGHT_EYE_OUTER")
        mouth_l_x, mouth_l_y = _pair("MOUTH_LEFT")
        mouth_r_x, mouth_r_y = _pair("MOUTH_RIGHT")
        ...
        lear_pairs = _make_pair(lear_x, lear_y)
        rear_pairs = _make_pair(rear_x, rear_y)
        leye_pairs = _make_pair(leye_x, leye_y)
        reye_pairs = _make_pair(reye_x, reye_y)
        leye_i_pairs = _make_pair(leye_i_x, leye_i_y)
        leye_o_pairs = _make_pair(leye_o_x, leye_o_y)
        reye_i_pairs = _make_pair(reye_i_x, reye_i_y)
        reye_o_pairs = _make_pair(reye_o_x, reye_o_y)
        mouth_l_pairs = _make_pair(mouth_l_x, mouth_l_y)
        mouth_r_pairs = _make_pair(mouth_r_x, mouth_r_y)
        ...
        unseen_hand = ~np.isfinite(np.asarray(disp_wr_x, dtype=float)) | ~np.isfinite(
            np.asarray(disp_wr_y, dtype=float)
        )
        if np.any(unseen_hand):
            for _fid, _jdata in finger_joints_arr.items():
                for _jname, _triple in list(_jdata.items()):
                    _ox, _oy, _cv = _triple
                    _ox = np.asarray(_ox, dtype=float).copy()
                    _oy = np.asarray(_oy, dtype=float).copy()
                    _ox[unseen_hand] = np.nan
                    _oy[unseen_hand] = np.nan
                    _jdata[_jname] = (_ox, _oy, _cv)
        ...
        # Table surface: frozen point under the rest affected shoulder.
        ...
        table_surface_y = table_surface_y_early
        if _is_seated_table_y(cream_y_early):
            table_surface_y = float(cream_y_early)
        elif table_under_shoulder and _is_seated_table_y(table_under_shoulder.get("y")):
            table_surface_y = float(table_under_shoulder["y"])
        if cup_y is not None and not _is_seated_table_y(table_surface_y):
            table_surface_y = float(cup_y)
        elif not _is_seated_table_y(table_surface_y):
            table_surface_y = _snap_table_y_to_rest_arm(
                table_surface_y,
                arm_support_y if arm_support_y is not None else rest_palm_y_early,
                rest_shoulder_y_early,
            )
            if not _is_seated_table_y(table_surface_y) and _is_seated_table_y(arm_support_y):
                table_surface_y = float(arm_support_y)
        if table_under_shoulder is not None and table_surface_y is not None:
            table_under_shoulder = {
                "x": table_under_shoulder.get("x"),
                "y": round(float(table_surface_y), 4),
            }
        table_surface_fallback = table_surface_y is not None and (
            table_edge is None or not (table_edge.get("under_shoulder"))
        )

        # Occlusion-robust shoulder Y: don't follow a dropped landmark while the hand is up.
        rest_shoulder_y_n = float(rest_shoulder_y_early) if np.isfinite(rest_shoulder_y_early) else float("nan")
        contra_y_n = np.asarray(ls_y if side == "right" else rs_y, dtype=float)
        rest_contra_y_n = float(np.nanmedian(contra_y_n[:rest_window])) if len(contra_y_n) else float("nan")
        robust_shoulder_y_px = np.asarray(shoulder_y, dtype=float) * frame_h
        if np.isfinite(rest_contra_y_n) and np.isfinite(rest_shoulder_y_n):
            expected_n = contra_y_n + (rest_shoulder_y_n - rest_contra_y_n)
            robust_n = np.minimum(np.asarray(shoulder_y, dtype=float), expected_n + 0.02)
        else:
            robust_n = np.asarray(shoulder_y, dtype=float)
        if np.isfinite(rest_palm_y_early) and np.isfinite(rest_shoulder_y_n):
            palm_raised = np.asarray(palm_y, dtype=float) < (rest_palm_y_early - 0.07)
            cap_n = rest_shoulder_y_n + 0.015
            robust_n = np.where(palm_raised, np.minimum(robust_n, cap_n), robust_n)
        robust_shoulder_y_px = robust_n * frame_h

        # Per-frame table-referenced shoulder elevation (robust Y vs frozen table).
        ...
        if table_surface_y is not None and np.isfinite(table_surface_y) and np.isfinite(rest_shoulder_y_px):
            ...
            if np.isfinite(denom) and denom > 1e-6:
                shoulder_elevation_table_ratio = (rest_shoulder_y_px - robust_shoulder_y_px) / denom

        # Fixed reference: rest shoulder x down to the table under that shoulder.
        shoulder_elevation_palm_ratio = np.full(len(new_t), np.nan)
        shoulder_palm_anchor = None
        if table_under_shoulder and table_under_shoulder.get("x") is not None:
            anchor_x = round(float(table_under_shoulder["x"]), 4)
            anchor_y = round(float(table_under_shoulder["y"]), 4)
        elif np.isfinite(rest_shoulder_x) and table_surface_y is not None and np.isfinite(table_surface_y):
            anchor_x = round(float(rest_shoulder_x), 4)
            anchor_y = round(float(table_surface_y), 4)
        else:
            anchor_x = None
            anchor_y = None
        if anchor_x is not None and anchor_y is not None and np.isfinite(rest_shoulder_y_px):
            anchor_y_px = float(anchor_y) * frame_h
            denom = abs(anchor_y_px - rest_shoulder_y_px)
            if np.isfinite(denom) and denom > 1e-6:
                shoulder_elevation_palm_ratio = (rest_shoulder_y_px - robust_shoulder_y_px) / denom
            shoulder_palm_anchor = [anchor_x, anchor_y]
            if table_under_shoulder is None:
                table_under_shoulder = {"x": anchor_x, "y": anchor_y}
        ...
        for i in range(len(new_t)):
            ...
            for fid, jdata in finger_joints_arr.items():
                ...
                for jname in ("mcp", "ip", "tip"):
                    ...
                    finite = bool(np.isfinite(ox[i]) and np.isfinite(oy[i]))
                    # Emit coords whenever finite so the player can sticky-hold;
                    # vis only gates full-opacity drawing.
                    ok = bool(finite and cv[i] >= 0.35)
                    entry["vis"][jname] = ok
                    entry[jname] = [round(float(ox[i]), 6), round(float(oy[i]), 6)] if finite else None
            ...
            frames.append({
                ...
                "leye": leye_pairs[i],
                "reye": reye_pairs[i],
                "leye_inner": leye_i_pairs[i],
                "leye_outer": leye_o_pairs[i],
                "reye_inner": reye_i_pairs[i],
                "reye_outer": reye_o_pairs[i],
                "mouth_l": mouth_l_pairs[i],
                "mouth_r": mouth_r_pairs[i],
                ...
                "finger_vis": {
                    "thumb": bool(finger_vis["thumb"][i] >= 0.35),
                    "index": bool(finger_vis["index"][i] >= 0.35),
                    "middle": bool(finger_vis["middle"][i] >= 0.35),
                    "ring": bool(finger_vis["ring"][i] >= 0.35),
                    "pinky": bool(finger_vis["pinky"][i] >= 0.35),
                },
        ...
        if analysis:
            for k in [
                ...
                "task_complete", "task_completion_ratio",
                "nvp_reach", "nvp_transport", "nvp_drink", "nvp_return", "nvp_total",
                "lift_height_sw", "lift_height_px", "lift_height_cm",
                "drink_lift_height_sw", "drink_lift_height_cm",
                "straightness_reach", "pause_time_sec_reach", "number_of_stops_reach",
                "grasp_dwell_sec", "grasp_dwell_stops", "functional_hold_sec",
                "sip_bout_count", "nvp_full_task", "nvp_excludes_sip_bouts", "primary_smoothness_window",
                "pause_time_sec_total", "number_of_stops_total",
                "pause_time_sec_path", "number_of_stops_path",
                "expected_phase_ids", "completed_phase_ids",
        ...
        if fs := float(analysis.get("analysis_fs_hz", analysis.get("fs_hz", target_fs))) if analysis else target_fs:
            cm_pp = None
            if analysis and analysis.get("cm_per_px"):
                try:
                    cm_pp = float(analysis["cm_per_px"])
                except (TypeError, ValueError):
                    cm_pp = None
            if cm_pp and cm_pp > 0:
                v_vals = [float(v) * cm_pp if np.isfinite(v) else 0.0 for v in speed]
                velocity_profile = {
        ...
        return {
            ...
            **({"adl_window": adl_window} if adl_window else {}),
            ...
            "peak_velocity_cm_s": (
                round(float(metrics["peak_velocity_cm_s"]), 2)
                if metrics and metrics.get("peak_velocity_cm_s") is not None
                else None
            ),
            ...
            "table_under_shoulder": table_under_shoulder,
            "cup": overlay_cup,
            "table_edge_y": (
                _pack_table_edge_profile(np.asarray(table_edge["edge_y"], dtype=float))
                if table_edge is not None and table_edge.get("edge_y") is not None
                else None
            ),
