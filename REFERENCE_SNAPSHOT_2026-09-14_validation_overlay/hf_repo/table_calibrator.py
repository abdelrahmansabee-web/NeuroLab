...
Physical table width = 85 cm (clinic table in front of the patient).
Converts pixel displacements to cm via:
...
def find_video_for_csv(csv_path: str, explicit_video: Optional[str] = None) -> Optional[Path]:
    ...
    for ext in (".mp4", ".mov", ".avi", ".mkv", ".webm"):
        for candidate in (
            csv_p.with_suffix(ext),
            csv_p.parent / f"{stem}{ext}",
            csv_p.parent / f"{stem_clean}{ext}",
            csv_p.parent.parent / f"{stem}{ext}",
            csv_p.parent.parent / f"{stem_clean}{ext}",
        ):
...
def estimate_table_width_px_from_wipe(
    ...
    *,
    ...
    """
    Reach-wipe: lateral palm span during the trial approximates table width (85 cm).
...
def calibrate_table_scale(
    ...
    *,
    ...
    """
    Return cm/px scale anchored to 85 cm table width.
