...
def restore_hf(hf_root: Path) -> int:
    ...
    copy_file(
        live / "static" / "js" / JS,
        hf_root / "frontend" / "build" / "static" / "js" / JS,
    )
    map_js = live / "static" / "js" / f"{JS}.map"
    if map_js.is_file():
        copy_file(map_js, hf_root / "frontend" / "build" / "static" / "js" / f"{JS}.map")
    copy_file(
        live / "static" / "css" / CSS,
        hf_root / "frontend" / "build" / "static" / "css" / CSS,
    )
    map_css = live / "static" / "css" / f"{CSS}.map"
    if map_css.is_file():
        copy_file(map_css, hf_root / "frontend" / "build" / "static" / "css" / f"{CSS}.map")
