...
def render_skeleton_validation_video(
    ...
    # Normalize columns for the procedural skeleton renderer
    try:
        from unified_validation_renderer import _normalize_landmark_columns
        df = _normalize_landmark_columns(df)
    except Exception:
        pass
    ...
    while True:
        ...
        if row is not None:
            try:
                from unified_validation_renderer import _draw_mediapipe_skeleton
                ...
                print(f"Procedural skeleton overlay failed: {exc}; falling back to wireframe")
...
def _build_coords_from_raw(
    ...
    *,
    ...
    # For side-view clinical videos, MediaPipe names landmarks from the *camera's*
    # point of view, not the person's.  The "active arm" (the one that moves most)
    # is the clinically relevant arm.  We therefore use the active arm's landmarks
    # for analysis, regardless of the user-provided affected_side label.
    active_side = detect_active_arm(raw, frame_width, frame_height, fs=infer_fps_from_df(raw))
    side = active_side if active_side in ("left", "right") else (affected_side.lower() if affected_side.lower() in ("left", "right") else "right")
    ...
    return {
        "affected_side": affected_side if affected_side.lower() in ("left", "right") else side,
        "active_landmark_side": side,
        "palm_x": palm_x,
        "palm_y": palm_y,
        "wrist_x": wrist_x,
        "wrist_y": wrist_y,
...
def _resample_analysis_df(
    df: pd.DataFrame,
    target_points: int = 1500,
) -> pd.DataFrame:
    """Resample analysis dataframe to a fixed number of points using cubic spline.

    Literature (Wagh et al. 2024) uses 1500 resampled points for fine upper-limb
    trajectory comparison; we expose the resampled series as an intermediate artifact.
    """
    if len(df) < 4 or target_points < 2:
        return df.copy()
    from scipy.interpolate import CubicSpline
    t = np.linspace(0, 1, len(df))
    t_new = np.linspace(0, 1, target_points)
    out = pd.DataFrame(index=np.arange(target_points))
    for col in df.columns:
        vals = pd.to_numeric(df[col], errors="coerce").values
        if np.isnan(vals).all():
            out[col] = vals[:target_points] if len(vals) >= target_points else np.pad(vals, (0, target_points - len(vals)), constant_values=np.nan)
            continue
        # Fill small internal gaps for interpolation stability
        mask = np.isfinite(vals)
        if mask.sum() < 4:
            out[col] = np.full(target_points, np.nan)
            continue
        cs = CubicSpline(t[mask], vals[mask])
        out[col] = cs(t_new)
    return out
...
def extract_from_video(
    ...
    if legacy_format:
        # Legacy raw output already matches the original Zeinab CSV format;
        # the analysis-ready CSV is identical to the raw CSV.
        analysis_df = raw.copy()
        analysis_df.to_csv(output_path, index=False)

        # Legacy still benefits from a filtered/resampled audit trail.
        filtered_path = inter_dir / f"{output_path.stem}_filtered_landmarks.csv"
        try:
            filt_df, _ = build_analysis_dataframe(
                raw,
            ...
            filt_df.to_csv(filtered_path, index=False)
        except Exception:
            filtered_path = None

        meta = {
            "camera_view": camera_view if camera_view != "auto" else "oblique",
            "affected_side": affected_side if affected_side != "auto" else "left",
            "shoulder_width_px": None,
            "fps": round(fps, 3),
            "frame_width_px": frame_width,
            "frame_height_px": frame_height,
            "butterworth_cutoff_hz": None,
            "butterworth_order": None,
            "velocity_threshold_px_s": default_velocity_threshold(frame_height),
            "percent_good_frames": None,
            "mean_palm_visibility": None,
            "landmark_tracking_enhanced": False,
            "trunk_proxy": "shoulder_girdle_midpoint",
            "legacy_format": True,
        }
        report: Dict[str, Any] = {
            ...
            "filtered_landmarks_csv": str(filtered_path) if filtered_path and filtered_path.exists() else None,
            "intermediate_dir": str(inter_dir),
            "frames": len(analysis_df),
            "extractor": "mediapipe_csv_extractor_legacy",
            **pose_report,
            **meta,
        }
        ...
        if filtered_path and filtered_path.exists():
            print(f"✓ Filtered landmarks CSV: {filtered_path}")
        print(f"  Camera view: {meta['camera_view']} | Side: {meta['affected_side']} | Legacy mode")
    ...
    resampled_path = None
    if save_resampled:
        resampled_path = inter_dir / f"{output_path.stem}_resampled_{resample_points}.csv"
        try:
            resampled_df = _resample_analysis_df(analysis_df, target_points=resample_points)
            resampled_df.to_csv(resampled_path, index=False)
        except Exception as exc:
            print(f"  Resampled CSV skipped: {exc}")
            resampled_path = None
    ...
    report: Dict[str, Any] = {
        ...
        "output_csv": str(output_path),
        "raw_pose_csv": str(raw_path) if save_raw_pose else None,
        "filtered_landmarks_csv": str(filtered_path),
        "resampled_landmarks_csv": str(resampled_path) if resampled_path and resampled_path.exists() else None,
        "intermediate_dir": str(inter_dir),
        "frames": len(analysis_df),
    ...
    if save_raw_pose:
        print(f"✓ Raw pose CSV: {raw_path}")
    print(f"✓ Filtered landmarks CSV: {filtered_path}")
    if resampled_path and resampled_path.exists():
        print(f"✓ Resampled landmarks CSV: {resampled_path}")
...
def main():
    ...
    parser.add_argument("--legacy", action="store_true", help="Use pose_extractor.py-compatible output format")
    ...
    parser.add_argument("--save-intermediate-frames", action="store_true", help="Save sampled frames used for detection")
