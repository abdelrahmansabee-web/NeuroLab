...
def analyze_neuro_session(
    ...
    try:
        from neuro_kinematics import analyze_patient_session, kinematic_values_to_api

        comparison, side_info, pre_kv, post_kv, healthy_kv = analyze_patient_session(
            healthy_path, post_path, pre_path
        )
        return {
            "comparison": comparison,
            "side_detection": side_info.to_dict(),
            "pre": kinematic_values_to_api(pre_kv, side_info.affected_side),
            "post": kinematic_values_to_api(post_kv, side_info.affected_side),
            "healthy": kinematic_values_to_api(healthy_kv, side_info.healthy_side),
            "baseline": kinematic_values_to_api(healthy_kv, side_info.healthy_side),
        }
    except Exception as e:
        traceback.print_exc()
        return {"error": str(e)}
