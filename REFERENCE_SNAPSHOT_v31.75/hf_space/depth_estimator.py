"""Metric scale estimator via ZoeDepth (lazy-loaded — not imported at app startup).
...
def _load_model():
    global _proc, _model, _torch
    if _model is not None:
        return True
    try:
        print("[depth] Loading ZoeDepth (first call only)...")
        import torch
        from PIL import Image  # noqa: F401 — used indirectly via processor
        from transformers import AutoImageProcessor, AutoModelForDepthEstimation
    except ImportError:
        print("[depth] torch/transformers not installed — metric depth disabled")
        return False
