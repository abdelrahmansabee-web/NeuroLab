...
def render_unified_validation_video(
    ...
    # Ensure table surface is available for the shoulder anchor fallback.
    # The palm at rest is on the table surface, so it is the most reliable reference.
    if analysis.get("table_surface_y") is None or not np.isfinite(float(analysis.get("table_surface_y"))):
        try:
            if "palm_y" in df.columns and len(df) > 0:
                palm_y_rest = float(df["palm_y"].iloc[0])
                if np.isfinite(palm_y_rest):
                    analysis["table_surface_y"] = float(palm_y_rest)
        except Exception:
            pass
    ...
    summary = {
        ...
        "shoulder_elevation_table_ratio": _safe_float(analysis.get("shoulder_elevation_table_ratio")),
        "shoulder_elevation_palm_ratio": _safe_float(analysis.get("shoulder_elevation_palm_ratio")),
    ...
    # Fixed wrist anchor point on the affected side at rest. The reference line
    # runs from the shoulder to the wrist (under the forearm).
    shoulder_palm_anchor = None
    if start_idx < n_rows and "wrist_x" in df.columns and "wrist_y" in df.columns:
        try:
            anchor_x = int(float(df["wrist_x"].iloc[start_idx]) * scale)
            anchor_y = int(float(df["wrist_y"].iloc[start_idx]) * scale)
    ...
    for frame_i in range(n_frames_video):
        ...
        # Draw reference line from the shoulder to the palm/table point (fixed at rest).
        if shoulder_palm_anchor and sh_pt:
            ax, ay = shoulder_palm_anchor
            sx, sy = sh_pt
            cv2.line(canvas, (sx, sy), (ax, ay), (236, 72, 153), _s(2), lineType=cv2.LINE_AA)
            cv2.circle(canvas, (ax, ay), _s(8), (236, 72, 153), -1, lineType=cv2.LINE_AA)
            cv2.circle(canvas, (ax, ay), _s(10), (255, 255, 255), 2, lineType=cv2.LINE_AA)
            _draw_label_near_pil(canvas, (ax, ay), "Table anchor", (236, 72, 153), offset=(_s(12), _s(-18)), orig_w=orig_w, orig_h=orig_h)

        if sh_pt:
            if summary.get("shoulder_elevation_palm_ratio") is not None and summary["shoulder_elevation_palm_ratio"] > 0:
                elev_text = f"Elev {summary['shoulder_elevation_palm_ratio']:.2f} PA"
            else:
                elev_text = f"Elev {summary['shoulder_elevation_norm']:.2f} SW"
            _draw_label_near_pil(canvas, sh_pt, elev_text, (255, 180, 100), offset=(_s(-10), _s(-14)), orig_w=orig_w, orig_h=orig_h)
        if el_pt:
