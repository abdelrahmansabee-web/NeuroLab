...
def _compute_elbow_angle(df: pd.DataFrame) -> np.ndarray:
    ...
    wy = pd.to_numeric(df["wrist_y"], errors="coerce").values
...
def compute_unified_kinematic_metrics(
    ...
    try:
        ...
        try:
            from stroke_kinematic_pipeline import _resolve_affected_side

            resolved_side = (
                affected_side
                if affected_side in ("left", "right")
                else _resolve_affected_side(raw_df, affected_side)
            )
        except Exception:
            resolved_side = affected_side if affected_side in ("left", "right") else "auto"

        df = load_canonical_landmarks(
            csv_path,
            affected_side=resolved_side,
            target_fs=target_fs,
            cutoff_hz=cutoff_hz,
