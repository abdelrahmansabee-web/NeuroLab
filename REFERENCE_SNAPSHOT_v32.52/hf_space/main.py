...
async def serve_static(rest_of_path: str):
    ...
    if file_path.exists() and file_path.is_file():
        # Hashed CRA assets: long cache so iPad PWA reopen does not re-download ~2MB main.js
        # every time when the Space is already warm. index.html stays no-cache.
        return FileResponse(
            file_path,
            headers={
...
def _write_patients_file(file: Path, patients: list) -> None:
    ...
    # Durable plaintext clinic snapshot (survives Space rebuild if /data is mounted).
    try:
        clinic = DATA_DIR / "patients.json"
        clinic.parent.mkdir(parents=True, exist_ok=True)
        clinic.write_text(json.dumps(patients, ensure_ascii=False, indent=2), encoding="utf-8")
        uid = "1"
        try:
            stem = file.stem
            if stem.isdigit():
            ...
            pass
        snap = DATA_DIR / f"neurolab_patients_{uid}.json"
        snap.write_text(
            json.dumps(
                {"patients": patients, "updatedAt": datetime.utcnow().isoformat() + "Z"},
                ensure_ascii=False,
                indent=2,
            ),
            encoding="utf-8",
        ...
        print(f"Clinic patients snapshot skipped: {exc}", flush=True)


@app.get("/api/patients")
async def api_get_patients(user: dict = Depends(get_current_user) if get_current_user else None):
    patients = _read_patients_file(_patients_file_for_user(user))
    try:
        from patient_drive_archive import load_program_patients, merge_patients

        # Merge clinic disk + Home Screen iPad backup so renamed-origin clients restore data.
        return merge_patients(patients, load_program_patients(DATA_DIR))
    except Exception:
        try:
            from patient_drive_archive import merge_patients

            return merge_patients(patients)
        except Exception:
            return patients
...
def _run_uv_generation(job_id: str, csv_path: Path, video_path: Path, rotation: str, patient_key: str = "", phase: str = ""):
...
def _overlay_cache_stale(cached: dict, analysis: Optional[Dict[str, Any]], video_path: Optional[Path]) -> bool:
    ...
    from overlay_data import OVERLAY_VERSION

    if int(cached.get("overlay_version") or 0) < int(OVERLAY_VERSION):
        return True
...
async def save_patients(body: dict, user: dict = Depends(get_current_user) if get_current_user else None):
    try:
        ...
        try:
            ...
            print(f"Patient dedupe skipped: {merge_exc}", flush=True)
        dest = _patients_file_for_user(user)
        # Never let an empty client wipe a non-empty durable DB (common after Space rename).
        if not patients:
            existing = _read_patients_file(dest)
            try:
                from patient_drive_archive import load_program_patients, merge_patients

                existing = merge_patients(existing, load_program_patients(DATA_DIR))
            except Exception:
                pass
            if existing:
                return {
        ...
        _write_patients_file(dest, patients)
        try:
            import threading
            ...
            def _reconcile_after_save():
                try:
                    reconcile_drive_to_program(patients)
                except Exception as exc:
                    print(f"Drive reconcile after save: {exc}", flush=True)
                    try:
                        from patient_drive_archive import archive_patients

                        archive_patients(patients, user_id=uid)
                    except Exception as arch_exc:
                        print(f"Drive patient archive: {arch_exc}", flush=True)

            threading.Thread(target=_reconcile_after_save, daemon=True).start()
        except Exception as exc:
...
<head>
<meta charset="utf-8">
<title>RA.ED AI PDF Parser Debug</title>
<style>
...
<body>
<h1>RA.ED AI PDF Import Debug</h1>
...
async def serve_build_root(file_name: str):
    ...
    if file_path.exists() and file_path.is_file():
        ...
        # Version-query assets (clinic_touch, pwa sync) + hashed media: cache briefly.
        # Keep HTML/manifest no-cache via dedicated routes / client meta.
        if name.endswith((".js", ".css", ".png", ".jpg", ".jpeg", ".webp", ".ico", ".woff2")):
            return FileResponse(
                file_path,
