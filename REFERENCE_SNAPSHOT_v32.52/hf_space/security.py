    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if ip in self.blocklist:
            if self.audit_db_path:
                try:
                    asyncio.create_task(asyncio.to_thread(self._log_block, ip, "IP in blocklist"))
                except Exception:
                    pass
        ...
        if content_length:
            try:
                if int(content_length) > self.max_content_length_bytes:
                    if self.audit_db_path:
                        try:
                            asyncio.create_task(asyncio.to_thread(self._log_block, ip, "Content-Length too large"))
                        except Exception:
                            pass
        ...
        for pattern in self._PATTERNS:
            if pattern.search(text):
                if self.audit_db_path:
                    try:
                        asyncio.create_task(asyncio.to_thread(self._log_block, ip, f"Pattern match: {pattern.pattern}"))
                    except Exception:
                        pass
                return Response("Forbidden", status_code=403)
