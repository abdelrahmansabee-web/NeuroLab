/**
 * Import patient session from RA.ED AI JSON export or Clinical Report PDF.
 */
...
const KIN_PDF_VARS = [
  { re: /NVP|Number of Velocity Peaks/i, key: "nvp" },
  { re: /Straightness/i, key: "straightness" },
  { re: /Pause Time/i, key: "pause_time_sec" },
  { re: /Number of Stops/i, key: "number_of_stops" },
  { re: /Trunk\s*Ratio|Trunk\/Palm/i, key: "trunk_ratio" },
  { re: /Shoulder Vert|Shoulder elevation/i, key: "shoulder_elevation_cm" },
  { re: /Peak (hand )?velocity|Peak Velocity/i, key: "peak_velocity_cm_s" },
  { re: /Elbow angle/i, key: "elbow_angle_mean_deg" },
  { re: /Shoulder flexion \(mean\)|Mean shoulder flexion/i, key: "shoulder_flexion_mean_deg" },
  { re: /Peak shoulder flexion|Peak shoulder flex/i, key: "peak_shoulder_flexion_vel_deg_s" },
  { re: /Movement time|Duration/i, key: "movement_time_sec" },
  { re: /Peak elbow angular velocity/i, key: "peak_elbow_ang_vel_deg_s" },
];
...
/** Normalize one patient record to RA.ED AI fd shape. */
export function normalizeImportedPatient(raw) {
  ...
  ["participantId", "group", "age", "sex", "strokeType", "side", "mas", "mrc", "height", "weight", "timeSinceStroke"].forEach((f) => {
...
export function parsePatientJson(text) {
  ...
  if (!raw.demographics && !raw.ipaq && !raw.vas) {
    throw new Error("Unrecognized JSON — export from RA.ED AI Report section");
  }
...
/** Parse RA.ED AI Clinical Assessment Report PDF text. */
...
export async function importPatientFile(file) {
  ...
  throw new Error("Supported formats: .json (RA.ED AI export) or .pdf (Clinical Report)");
}
