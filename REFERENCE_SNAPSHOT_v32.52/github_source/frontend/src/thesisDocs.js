...
*RA.ED AI / Istinye University — ready for thesis committee & manuscript paste-up*
...
Ordinal scales (e.g., FMA) cannot capture subtle trajectory quality (Massie et al., 2012). **Markerless pipelines** (MediaPipe + monocular depth scaling in RA.ED AI) enable clinic-feasible, objective metrics: SPARC smoothness, trunk-to-palm ratio, shoulder elevation, elbow angle, movement time, and peak velocity (Schwarz et al., 2022).
...
| Ordinal clinical endpoints only | RA.ED AI objective kinematics + WMFT-4 |
...
---
*Generated by RA.ED AI Analysis Dashboard — align variable names with \`analysisPlan.js\`.*
`;
}
...
*For thesis committee review — RA.ED AI v6*
...
| Level | Variable | SPSS columns | Direction | Test |
|-------|----------|--------------|-----------|------|
| A | Task complete | \`task_complete_Pre\`, \`task_complete_Post\` | Higher = better | Mixed / McNemar |
| B | NVP (reach) | \`nvp_reach\` | Lower = better | 2×2 Mixed ANOVA |
| B | NVP (drink lift) | \`nvp_drink\` | Lower = better | Descriptive / secondary |
| B | NVP (return) | \`nvp_return\` | Lower = better | Descriptive / secondary |
| B | NVP (total phases) | \`nvp_total\` | Lower = better | Exploratory sum |
| B | Drink lift height | \`drink_lift_height_cm\` | Higher = greater lift (table width = 85 cm scale) | Descriptive |

Phase NVPs are computed on reach, table→peak drink lift, and return. \`nvp_total\` = sum of available phase NVPs. Extra sips are still counted only in \`sip_bout_count\` / exploratory full-task NVP.
| B | Path straightness (reach) | \`straightness_reach_Pre/Post\` | Higher = better | 2×2 Mixed ANOVA |
| B | Pause time (reach, path) | \`pause_time_sec_reach_Pre/Post\` | Lower = better | 2×2 Mixed ANOVA |
| B | Number of stops (reach, path) | \`number_of_stops_reach_Pre/Post\` | Lower = better | 2×2 Mixed ANOVA |

Extra drink sips are counted in \`sip_bout_count\` (descriptive) and optionally \`nvp_full_task\` (exploratory). They do **not** enter primary NVP/pause.

Terminal grasp fixation and mouth/face hold are stored as \`grasp_dwell_sec\` / \`functional_hold_sec\` (exploratory) and are **not** counted in path pause.

Full-task NVP (\`nvp_full_task\`) is **exploratory** only — do not treat extra peaks from more sips as worse movement.

### Secondary kinematic (5 tests — **Holm–Bonferroni**, k=5)
| # | Variable | Columns | Better |
|---|----------|---------|--------|
| 1 | Trunk ratio | trunk_ratio_Pre/Post | ↓ |
| 2 | Shoulder elevation | \`shoulder_elevation_cm\` | ↓ (table 85 cm scale) |
| 3 | Peak hand velocity | \`peak_velocity_cm_s\` | ↑ (table 85 cm scale) |
...
6. **Missing data:** LOCF primary (export checkbox in RA.ED AI); Mixed Models sensitivity.  
7. **Task completion:** If Pre is incomplete and Post is complete, report completion first; compare smoothness only on the shared reach window (\`nvp_reach\`, \`pause_time_sec_reach\`). Do not treat higher full-task NVP or grasp dwell after a completed lift as deterioration.  
8. **Pause definition:** Primary pause/stops = path pauses only. Terminal grasp fixation and transport mouth hold are excluded from path pause and reported as functional hold.  
...
---
*SAP version 1.1 — RA.ED AI auto-generated.*
`;
}
