...
def analyze_reach_and_wipe(
    ...
    try:
        ...
        # ── Auto-detect the more active hand (based on peak velocity for task-specific detection) ──
        v_right = _tang_vel(rw_x, rw_y, fs)
        v_left  = _tang_vel(lw_x, lw_y, fs)
        right_path = _hand_total_path(rw_x, rw_y)
        left_path  = _hand_total_path(lw_x, lw_y)
        
        side = affected_side.lower()
        if side == "auto":
            side = "right" if v_right.max() >= v_left.max() else "left"
        ...
        active_shoulder_y = rs_y if side == "right" else ls_y
        ...
        gs_y         = active_shoulder_y[win]
        g_shoulder_vert = round((gs_y.max() - gs_y.min()) / ref_scale, 4)
        ...
        result = {
            "_code_version": "2026-06-09-v4",  # diagnostic marker
            # QC / metadata
            "side_analyzed":        side,
            ...
            "shoulder_vert_norm":     g_shoulder_vert,
        ...
        if metric_scale > 0:
            ...
            result["total_depression_cm"]  = round(g_shoulder_vert * m2cm, 2)
