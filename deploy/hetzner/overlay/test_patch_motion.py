...
JS_SAMPLE = "".join(old for _label, old, _new in JS_PATCHES)
CSS_SAMPLE = "".join(old for _label, old, _new in CSS_PATCHES)
...
class MotionPatchTests(unittest.TestCase):
    ...
    def test_applies_to_live_clinic_bundle(self) -> None:
        ...
        js_text = bundle.read_text(encoding="utf-8", errors="replace")
        css_text = css.read_text(encoding="utf-8", errors="replace")
        js_out, js_applied = patch_js_text(js_text)
