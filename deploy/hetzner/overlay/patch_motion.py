#!/usr/bin/env python3
"""Sidebar slides on GPU; topbar/main keep the right edge on screen.

Section changes use the live KS exit/enter path with a short bounce-in.
Do not translate a 100% wide topbar/main — that clips New/Save/More.
"""
...
KIN = "20"
NL_VERSION = "31.92"
START_URL = "./?v=29.71-pwa"
DEPLOY_VERSION = "29.71"

PAD_EASE = (
    "left 600ms cubic-bezier(0.33, 0, 0.2, 1),"
    "margin-left 600ms cubic-bezier(0.33, 0, 0.2, 1)"
)
...
JS_PATCHES = (
    (
        "topbar and main ease left/margin, not a full-width translate",
        (
    ...
    (
        "pin the topbar right edge so actions stay on screen",
        (
            TOPBAR_PAD,
            TOPBAR_GPU,
        ),
    ...
    (
        "pin the main right edge so the form is not clipped",
        (
            MAIN_PAD,
            MAIN_GPU,
        ),
    ...
    (
        "keep section bounce even if the OS asks for reduced motion",
        (
    ...
    (
        "play a short bounce when changing sections",
        (
            NAV_INSTANT,
            NAV_EXITING_280,
        ),
    ...
    (
        "keep enter bounce fallback in sync with CSS",
        (
...
def patch_pwa_reload(root: Path) -> list[str]:
    ...
    if idx.is_file():
        ...
        if updated != html:
            ...
            notes.append(f"index kin={KIN} css?m=8 nl-version {NL_VERSION}")
