...
def _nan_smooth(arr: np.ndarray, win: int) -> np.ndarray:
    ...
    pad = k // 2
    filled = x.copy()
    last = np.nanmedian(filled[np.isfinite(filled)]) if np.any(np.isfinite(filled)) else 0.0
    for i in range(n):
...
def _lift_split_windows(
    ...
    """
    Split one continuous bout into reach / transport / return using palm height.

    Image y increases downward. A drink/brush has table → mouth (y drop) → table.
    Slow Pre/Post clips often never fall below the speed threshold three times.
    """
    if len(phase_ids) < 3:
        return []
    ...
    if end - start < max(12, int(0.4 * fs)):
        return []
    y = np.asarray(palm_y[start : end + 1], dtype=float)
    ys = _nan_smooth(y, max(3, int(0.08 * fs)))
    if not np.any(np.isfinite(ys)):
        return []
    head_n = max(3, len(ys) // 8)
    base = float(np.nanmedian(ys[:head_n]))
    peak_i = int(np.nanargmin(ys))
    peak = float(ys[peak_i])
    if not np.isfinite(base) or not np.isfinite(peak):
        return []
    lift_px = float(base - peak)
    sw = float(shoulder_width) if shoulder_width and shoulder_width > 0 else 50.0
    min_lift = max(6.0, 0.08 * sw)
    if lift_px < min_lift:
        return []
    tail = ys[peak_i:]
    if tail.size < max(4, int(0.12 * fs)):
        return []
    rest_n = max(3, len(tail) // 6)
    rest = float(np.nanmax(tail[-rest_n:]))
    if not np.isfinite(rest) or (rest - peak) < 0.32 * lift_px:
        return []

    drop_thr = base - 0.28 * lift_px
    rec_thr = peak + 0.28 * lift_px
    t0 = peak_i
    while t0 > 0 and ys[t0] <= drop_thr:
        t0 -= 1
    t1 = peak_i
    while t1 < len(ys) - 1 and ys[t1] <= rec_thr:
        t1 += 1

    a0, a1 = start, start + int(t0)
    b0, b1 = start + int(t0), start + int(t1)
    c0, c1 = start + int(t1), end
    min_f = max(4, int(0.10 * fs))
    if (a1 - a0) < min_f:
        a1 = min(end, a0 + min_f)
        b0 = a1
    if (b1 - b0) < min_f:
    ...
    if (c1 - c0) < min_f:
        c0 = max(b1, end - min_f)
    if not (a0 < a1 <= b1 <= c1 and b0 < b1 and c0 < c1):
        return []
    transport_id = phase_ids[1]
    return [
        (phase_ids[0], int(a0), int(a1)),
        (transport_id, int(b0), int(b1)),
        (phase_ids[-1], int(c0), int(c1)),
    ]
...
def _task_completion_fields(
    ...
    if spec.get("multi_bout") and not detected:
        complete = False
    # Spatial fallback: a real sip/brush has lift + return even when speed
    # bouts collapsed into 1–2 fragments (common on slow Pre/Post).
    if spec.get("multi_bout") and not complete:
        has_transport = any(str(p.get("id") or "").startswith("transport_") for p in (phases_out or []))
        has_return = any(str(p.get("id") or "") == "return" for p in (phases_out or []))
        has_lift = False
        for p in phases_out or []:
            m = p.get("metrics") or {}
            try:
                sw_v = m.get("lift_height_sw")
                px_v = m.get("lift_height_px")
                if sw_v is not None and float(sw_v) >= 0.05:
                    has_lift = True
                if px_v is not None and float(px_v) >= 6.0:
                    has_lift = True
            except (TypeError, ValueError):
                pass
        if has_lift and has_return:
            complete = True
            for pid in expected:
                if pid not in detected:
                    detected.append(pid)
        elif has_transport and has_return:
            complete = True
            for pid in expected:
                if pid not in detected:
                    detected.append(pid)
...
def analyze_clinical_task_phases(
    *,
    ...
    spd = palm_image_speed(palm_x, palm_y, fs)
    # Slow stroke Pre/Post often never exceeds 5 px/s three separate times.
    # Keep NVP thresholds unchanged; only the ADL bout splitter is more sensitive.
    bout_thr = min(float(velocity_threshold), max(1.6, float(velocity_threshold) * 0.4))
    segments = _list_segments(
        ...
        fs,
        bout_thr,
        max_gap_frames=max(8, int(0.18 * fs)),
        min_segment_frames=max(6, int(0.12 * fs)),
    )
    sw = float(shoulder_width) if shoulder_width and shoulder_width > 0 else 50.0
    segments = [s for s in segments if s["dur"] >= 0.15 and (s["disp"] / sw) >= 0.02]

    windows = _pick_phase_windows(segments, spec["phase_ids"])
    if len(spec["phase_ids"]) >= 3:
        have = {w[0] for w in windows}
        if not all(pid in have for pid in spec["phase_ids"]):
            if segments:
                span_s = int(segments[0]["start"])
                span_e = int(segments[-1]["end"])
            else:
                span_s, span_e = 0, max(0, len(palm_y) - 1)
            split = _lift_split_windows(
                palm_y, span_s, span_e, fs, shoulder_width, spec["phase_ids"]
            )
            if split:
                windows = split
            elif not segments:
                split_full = _lift_split_windows(
                    palm_y, 0, max(0, len(palm_y) - 1), fs, shoulder_width, spec["phase_ids"]
                )
                if split_full:
                    windows = split_full
    phases_out: List[Dict[str, Any]] = []
    ...
    if len(spec["phase_ids"]) >= 3 and phases_out:
        has_transport = any(str(p.get("id") or "").startswith("transport_") for p in phases_out)
        if not has_transport:
            s0 = int(phases_out[0].get("start_frame") or 0)
            e1 = int(phases_out[-1].get("end_frame") or s0)
            span_lift = _palm_lift_height_sw(palm_y, s0, e1, shoulder_width)
            if span_lift:
                phases_out[-1].setdefault("metrics", {}).update(span_lift)
