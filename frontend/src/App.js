...
import {
  ...
  RECOVERY_SUMMARY_KEYS, kinCrossPhaseComparable, computeOverlayMetrics,
} from "./analysisPlan";
import {
    ...
    return (
      <div className="relative w-full rounded-lg bg-black overflow-hidden group">
        <video
          ...
          onClick={togglePlay}
        />
        <div className="absolute inset-x-0 bottom-0 p-2 bg-gradient-to-t from-black/80 to-transparent opacity-0 
group-hover:opacity-100 transition-opacity pointer-events-none">
          <div className="flex items-center gap-2 pointer-events-auto">
            <button
              type="button"
              onClick={togglePlay}
              className="text-white/90 hover:text-white text-xs font-bold px-2 py-1 rounded bg-white/10 
hover:bg-white/20"
            >
              {isPlaying ? "ΓÅ╕ Pause" : "Γû╢ Play"}
            </button>
            <button
              type="button"
...
function ValidationOverlayPlayer({ videoFile, overlayData, phaseLabel, autoPlay = false, onEnded, onDownloadReady }) {
  ...
  const peakV = overlayData?.peak_velocity_px_s || metrics.peak_velocity_px_s || 1;
  ...
  const drawOverlay = useCallback(() => {
    ...
    function pillWithArrow(text, p, opts = {}) {
      ...
      // Clamp label position inside canvas
      let lx = p[0] + offsetX;
      let ly = p[1] + offsetY;
      ...
      const cx = lx + w / 2;
      const cy = ly - h / 2;
      // Draw connecting line from label center to landmark
      ctx.save();
      ctx.strokeStyle = border;
      ctx.lineWidth = 1.5;
      ctx.setLineDash([2, 2]);
    ...
    // All skeleton joints (bone-like circles)
    [
      nose, ls, rs, le, re, lw, rw, lh, rh, lk, rk, la, ra, trunk,
    ].forEach((p) => {
      if (p) dot(p, { fill: jointFill, stroke: jointOutline, r: 9 });
    });

    // Affected arm highlighted in clinical accent (still visible over the skeleton)
    ...
    // ---- Per-frame animated metric calculations ----
    const speedThreshold = peakV > 0 ? 0.05 * peakV : 1.0;
    ...
    // NVP up to current frame
    const currentNVP = peakFrames.filter((pi) => pi <= idx).length;

    // Peak velocity up to current frame
    ...
    // Pause time and stops up to current frame
    ...
    // Straightness up to current frame (displacement / path length)
    ...
    // Trunk ratio up to current frame
    ...
    // Shoulder elevation up to current frame (in shoulder-width units)
    let currentShoulderElevation = 0;
    if (inMovement && video.videoHeight && m.shoulder_width_px) {
      let shStart = null;
      let shMin = null;
      for (let i = win.start_idx; i <= idx && i < frames.length; i++) {
        const sh = frames[i].shoulder;
        if (!sh) continue;
        if (shStart === null) shStart = sh[1];
        if (shMin === null || sh[1] < shMin) shMin = sh[1];
      }
      if (shStart !== null && shMin !== null && m.shoulder_width_px > 0) {
        const elevPx = Math.abs(shStart - shMin) * video.videoHeight;
        currentShoulderElevation = elevPx / m.shoulder_width_px;
      }
    }

    // Current elbow angle and speed
    const currentElbowAngle = f.elbow_angle || 0;
    const currentSpeed = f.speed || 0;
    const wipingVerdict = f.wiping_verdict;
    const wiping = overlayData?.wiping || {};

    // ---- Simple labels at landmarks (no leader lines) ----
    ...
    // Counter labels at landmarks (values grow as the movement progresses)
    const cx = cw / 2;
    if (shoulder) {
      const shText = currentShoulderElevation > 0 ? `Sh ${currentShoulderElevation.toFixed(2)} SW` : "Sh";
    ...
    if (palm) {
      drawSimpleLabel(`Ha ${Math.round(currentSpeed)} px/s`, palm, palm[0] > cx ? -100 : 18, -24, { color: "#fde047", border: "rgba(250,204,21,0.6)" });
      // NVP counter under the yellow trajectory line
      drawSimpleLabel(`NVP ${currentNVP}`, palm, 0, 28, { color: color.text, border: color.glow, align: "center" });
      // Wiping verdict label under the NVP counter
      if (wipingVerdict) {
        drawSimpleLabel(
    ...
    // Start/End points and persistent straightness line
    const startPt = toCanvas(startPalm);
    const endPt = toCanvas(endPalm);
    // Cumulative actual hand path (green) - follows the real hand movement
    if (idx >= win.start_idx && idx <= win.end_idx) {
      ...
      // Progress dot at current palm position
      if (palm) {
    ...
    // Trajectory trace with gradient fade
    ...
    // ---- NVP counter is drawn under the yellow trajectory line on the video ----
    ...
    // Header: phase label on left, live NVP counter on right
    ...
    // Live animated metrics (values grow as the movement progresses)
    ...
    row("Peak velocity", currentPeakVelocity > 0 ? `${formatValue(currentPeakVelocity, 0)} px/s` : "—", true);
    ...
    // Live velocity chart up to current frame
    ...
    ctx.fillText(`Speed ${Math.round(speed)} px/s`, gx, gy - 4);

  }, [frames, fps, metrics, win, peakV, startPalm, endPalm, velocityProfile, phaseColor, phaseLabel, getFrameIndex, peakFrames, overlayData?.elbow_angle_profile, overlayData?.trunk_x_profile, overlayData?.wiping]);
...
const KinSection = ({ data, demographics, onChange, showToast, sessionKey }) => {
  ...
  const [overlayDataMap, setOverlayDataMap] = useState({});
  ...
  const analyzeVideo = async (phase) => {
    ...
    try {
      ...
      // Fetch lightweight overlay data immediately so the browser can render the
      // validation video without waiting for server-side encoding.
      if (result.overlay_data_url) {
        fetchOverlayData(phase, result.overlay_data_url);
      }
      // Keep the legacy background unified video generation for users who want a
      // server-rendered file, but the primary experience is now the overlay player.
  ...
  // Fetch lightweight per-frame overlay data for browser-side validation rendering.
  const fetchOverlayData = useCallback(async (phase, url) => {
    try {
      const res = await fetch(`${API_BASE}${url}`, { cache: "no-store" });
      if (!res.ok) throw new Error(`Overlay data failed (${res.status})`);
      const data = await res.json();
      if (data.error) throw new Error(data.error);
      setOverlayDataMap((prev) => ({ ...prev, [phase]: data }));
      // Merge overlay-computed metrics into kinematicsResults so the table/report match the video overlay.
      const overlayMetrics = computeOverlayMetrics(data);
      if (overlayMetrics) {
  ...
  // a ready overlay player, a server-rendered unified validation video, or a
  // permanent generation error. The overlay player is now the primary visual
  ...
  }, [kinematicsResults, uvErrors, overlayDataMap]);
  ...
  const getMetricValue = (phase, key) => {
    const result = kinematicsResults[phase];
    if (!result) return "—";
    // Prefer overlay-computed values (merged into result by fetchOverlayData) so
    // the table and the validation video panel show the same numbers.
    const overlayNum = pickKinField(result, key);
    if (overlayNum !== null) return overlayNum;
  ...
  const displayMetricValue = (phase, key, unit) => {
    const val = getMetricValue(phase, key);
    if (unit === "ratio" && typeof val === "number" && !Number.isNaN(val)) return val.toFixed(3);
    return formatKinValue(key, val);
  };

  const KIN_TIPS = {
    wiping_verdict: "Wiping quality verdict: smooth / moderate / jerky.",
    wiping_n_strokes: "Number of back-and-forth wiping strokes detected.",
    wiping_cv_speed: "Coefficient of variation of hand speed — lower = more consistent speed.",
    wiping_straightness: "Stroke straightness = straight-line displacement / actual path length.",
    wiping_coverage: "Area covered by the wiping hand (width × height in pixels²).",
    wiping_duration: "Active wiping duration after trimming static start/end frames.",
  ...
  const variables = orderedKinematicVars().map((v) => {
    let key = v.key;
    let unit = v.unit || "—";
    // When overlay data has computed video-style metrics, prefer the same units the video overlay uses.
    const hasVideoMetrics = Object.values(kinematicsResults).some((r) => r?.peak_velocity_px_s != null);
    if (hasVideoMetrics) {
      if (key === "peak_velocity_cm_s") {
        key = "peak_velocity_px_s";
        unit = "px/s";
      }
      if (key === "trunk_ratio") {
        unit = "ratio";
  ...
  const kinViewWarning = (() => {
    const lowAmp = activeResultPhases.filter((ph) => kinematicsResults[ph.k]?.wiping_comparable === false);
    if (lowAmp.length) {
        ...
        <div className="rounded-xl border border-white/[0.08] bg-white/[0.03] p-3">
          ...
          {kinResultsTab === "compare" ? (
              ...
              {activeResultPhases.map((ph) => (
                <div key={ph.k} className={`rounded-lg border px-2.5 py-2 ${phaseValueCls(ph.c)}`}>
                  ...
                  <p className="text-base font-mono font-bold text-white/90 mt-0.5">
                    {displayMetricValue(ph.k, metric.key, metric.unit)}
                  </p>
                </div>
              ))}
            </div>
            ...
            <p className="text-2xl font-mono font-extrabold text-white/90">
              {displayMetricValue(kinResultsTab, metric.key, metric.unit)}
            </p>
          )}
                ...
                <div className="px-4 py-2 flex flex-col flex-1 min-h-0">
                  ...
                  {hasResult && (
                      <div className="flex justify-center gap-1">
                        ...
                        <GBtn variant="default" onClick={() => kinematicsResults[ph.k]?.unified_validation_video ? downloadFile(ph.k, "unified") : generateUnifiedValidation(ph.k)} disabled={analysisStatus[ph.k] === "generating_unified" || overlayDataMap[ph.k]} className="!py-1.5 !px-2 min-w-[2.25rem] shrink-0" title={kinematicsResults[ph.k]?.unified_validation_video ? "Unified Validation Video" : overlayDataMap[ph.k] ? "Validation overlay ready" : "Generate Unified Validation Video"}>
                          ...
                          return (
                            <div key={key} className={`rounded-lg border px-2 py-1.5 ${phaseValueCls(ph.c)}`}>
                              ...
                              <p className="text-sm font-mono font-extrabold text-white/90 mt-0.5">
                                {displayMetricValue(ph.k, key, meta.unit)}
      ...
      {Object.keys(kinematicsResults).length > 0 && (
        <Glass className="p-4 sm:p-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {phases.filter((ph) => kinematicsResults[ph.k] && overlayDataMap[ph.k]).map((ph) => (
              <div key={ph.k} className={`rounded-xl border p-3 ${phaseValueCls(ph.c)}`}>
                ...
                <ValidationOverlayPlayer
                  videoFile={data[`${vidKey(ph.k)}_file`]}
                  overlayData={overlayDataMap[ph.k]}
                  phaseLabel={ph.l}
                  autoPlay
            ...
            {phases.filter((ph) => kinematicsResults[ph.k]?.unified_validation_video && !overlayDataMap[ph.k]).map((ph) => (
              <div key={ph.k} className={`rounded-xl border p-3 ${phaseValueCls(ph.c)}`}>
                <div className="flex items-center justify-between mb-2">
            ...
            {phases.filter((ph) => kinematicsResults[ph.k] && !overlayDataMap[ph.k] && !kinematicsResults[ph.k]?.unified_validation_video).map((ph) => (
              <div key={ph.k} className={`rounded-xl border p-3 ${phaseValueCls(ph.c)}`}>
...
const ReportSection = ({ fd, onChange, showToast }) => {
  ...
  const kinDirectionMap = (name) => {
    const n = (name || "").toLowerCase();
    if (n.includes("wiping_cv_speed")) return "lower";
    if (n.includes("wiping_straightness")) return "higher";
    if (n.includes("wiping_coverage")) return "higher";
    if (n.includes("wiping_n_strokes")) return "none";
