...
import { kinAnalysisResultsSig, kinAsyncStillCurrent, shouldResetKinSessionMedia } from "./kinSessionIdentity";
import {
  backupDriveArtifact,
...
const KinSection = React.memo(function KinSection({ data, demographics, onChange, showToast, sessionKey }) {
  ...
  const kinIdentityRef = useRef(sessionKey);
  ...
  // Reload kinematics when switching patient session. Drop live overlay/video
  // so a later Drive/IDB hydrate cannot keep the previous patient's clip.
  useEffect(() => {
    ...
    prevSessionKeyRef.current = next;
    ...
    if (resetMedia) {
      ...
      Object.values(originalVideoBlobsRef.current || {}).forEach((url) => {
        if (typeof url === "string" && url.startsWith("blob:")) URL.revokeObjectURL(url);
      });
      Object.values(videoBlobsRef.current || {}).forEach((url) => {
        if (typeof url === "string" && url.startsWith("blob:")) URL.revokeObjectURL(url);
      });
    ...
    try {
      ...
      if (patientCacheKey) {
        ...
        if (validationCacheMatchesResult(cached, { csv_filename: csvFilename }, { relaxCsvMatch: true }) && cached?.overlay?.frames?.length) {
          ...
          applyValidationCacheToState(phase, { overlay: cached.overlay }, started);
        } else {
  ...
  useEffect(() => {
    ...
    (async () => {
      for (const ph of phases) {
        if (cancelled || !kinWorkStillHere(started)) break;
  ...
  const loadVideoBlob = useCallback(async (phase, filename, { silent = false } = {}) => {
    ...
    const applyCachedUnified = async () => {
      ...
      applyValidationCacheToState(phase, { unifiedVideoBlob: blob }, started);
      return true;
    };
  ...
  const KIN_TIPS = {
    ...
    nvp_reach: "Velocity peaks on the reach window only. Peaks that fall in drink/return are counted there, not here.",
    ...
    nvp_total: "Sum of exclusive phase NVPs (reach + drink/transport + return). Each peak is counted once.",
    ...
    nvp_reach: "Velocity peaks on the reach window only, from the same peak_frames as the overlay. Drink/return peaks are not counted here.",
    ...
    tremor_8_12hz_power: "8–12 Hz on the same overlay palm path as Straightness (deviation from the start–end chord).",
    ...
    tremor_8_12hz_power: "8–12 Hz of the overlay palm path used for Straightness (not a separate speed channel). Lower = less tremor. Index/ADL tremor stay under Show all.",
