...
test("analysis stage capsule is muted glass and keeps the film-strip motion", () => {
  ...
  expect(css).toMatch(/\.kin-analyze-stage__capsule[\s\S]*?border-radius:\s*40px/);
  ...
  expect(css).toMatch(/\.kin-analyze-stage__capsule[\s\S]*?border-radius:\s*40px/);
  expect(css).toMatch(/\.kin-run__stage[\s\S]*?height:\s*148px/);
  expect(css).toMatch(/\.kin-run__figure[\s\S]*?width:\s*124px/);
  expect(css).toMatch(/\.kin-run__figure[\s\S]*?height:\s*130px/);
  expect(css).toMatch(/kin-run-pose 0\.42s steps\(1/);
  expect(css).toMatch(/\.kin-run__pct/);
  expect(app).toMatch(/kin-run__figure--\$\{id\}/);
  expect(app).toMatch(/kin-run__pct-num/);
  expect(app).toMatch(/<KinRunMark pct=\{pctRounded\} \/>/);
  expect(app).toMatch(/status === "analyzing"\) return `\$\{base\} overflow-visible/);
  ...
  expect(app).toMatch(/<KinRunMark pct=\{pctRounded\} \/>/);
  expect(app).toMatch(/static\/kin-run/);
  expect(app).toMatch(/pose-\$\{id\}\.png/);
  expect(app).not.toMatch(/kin-run__skull/);
  ...
  expect(html).toMatch(/if \(standalone\) return;/);
  expect(html).toMatch(/__nlAuthMeTimeout/);
  expect(sync).toMatch(/isIOS && !standalone/);
  expect(fs.readFileSync(path.join(__dirname, "..", "public", "manifest.json"), "utf8")).toMatch(/"start_url": "\.\/"/);
