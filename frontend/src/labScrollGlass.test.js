...
test("analysis stage capsule is muted glass and keeps the film-strip motion", () => {
  ...
  expect(css).toMatch(/\.kin-analyze-stage__capsule[\s\S]*?border-radius:\s*40px/);
  expect(css).toMatch(/\.kin-run__stage[\s\S]*?height:\s*276px/);
  expect(css).toMatch(/\.kin-run__figure[\s\S]*?width:\s*248px/);
  expect(css).toMatch(/\.kin-run__figure[\s\S]*?height:\s*260px/);
