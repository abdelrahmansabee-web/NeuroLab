...
(function () {
  ...
  /** Only the Space POST / long UV job — not progress polls (those go idle between ticks). */
  function isClinicJob(url) {
    var u = String(url || "");
    if (u.indexOf("/unified-validation") !== -1) return true;
    if (u.indexOf("/analyze-progress/") !== -1) return false;
    if (u.indexOf("/analyze-result/") !== -1) return false;
    return u.indexOf("/analyze") !== -1;
  }
    ...
    window.fetch = function () {
      ...
      return Promise.resolve(p).then(
        function (res) {
          ...
          if (busy === 0 && pendingAfterBusy) scheduleDeferredSync();
          return res;
        },
        function (err) {
          busy = Math.max(0, busy - 1);
  ...
  function uploadPatients() {
    ...
    try {
      ...
      // Fire-and-forget: do not chain UI on /api/ipad-localstorage (can be huge + Drive archive).
      quietFetch("/api/ipad-localstorage", {
        method: "POST",
        body: fd,
        credentials: "same-origin",
        headers: authHeaders(),
  ...
  function syncVideos(force) {
    lastForce = !!force;
    // Home Screen only: never compete with Analyze — leave/return hang returns otherwise.
    if (analyzeActive()) {
      pendingAfterBusy = true;
    ...
    if (!force) {
      var now = Date.now();
      if (now - lastAutoSyncAt < AUTO_SYNC_GAP_MS) return Promise.resolve();
      lastAutoSyncAt = now;
    }
    syncing = true;
    return openDb()
      ...
      .then(function (stats) {
        if (stats.deferred && (busy > 0 || analyzeActive())) return stats;
        // Defer heavy localStorage dump so first paint / clinic taps stay free.
        setTimeout(function () {
          if (!analyzeActive()) uploadPatients();
          else pendingAfterBusy = true;
        }, force ? 500 : 8000);
        return sendReport(stats).then(function () {
          return stats;
        });
      })
      .then(function (stats) {
        if (stats.deferred && (busy > 0 || analyzeActive())) {
          return;
        }
      ...
      .then(function () {
        syncing = false;
        // Analyze just cleared — run the deferred Drive sync once.
        if (pendingAfterBusy && !analyzeActive() && busy === 0) {
  ...
  function start() {
    // Well after React first paint; avoids competing with HF cold start + main.js.
    setTimeout(function () {
      if (!analyzeActive()) syncVideos(false);
      else pendingAfterBusy = true;
    }, 20000);
  }
  ...
  // sessionStorage is per-tab — poll so deferred Drive sync runs after Analyze clears.
  setInterval(function () {
    if (!pendingAfterBusy || analyzeActive() || busy > 0 || syncing) return;
    scheduleDeferredSync();
  }, 4000);
})();
