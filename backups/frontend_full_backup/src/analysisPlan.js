...
export const KINEMATIC_VARS = [
  ...
  { key: "elbow_angle_mean", label: "Elbow angle (mean)", unit: "deg", dir: "higher", tier: "secondary" },
  { key: "movement_time_sec", label: "Movement time", unit: "s", dir: "lower", tier: "secondary" },
  { key: "peak_velocity_px_s", label: "Peak velocity", unit: "px/s", dir: "higher", tier: "secondary" },
];
...
export const MANUSCRIPT_KINEMATIC_TARGETS = {
  ...
  elbow_angle_mean: { pre: 65.0, post: 85.0, healthy: 95.0 },
  peak_velocity_px_s: { pre: 45.0, post: 55.0, healthy: 62.2 },
};
...
export const SPSS_WORKFLOW = [
  ...
  { step: 6, title: "Secondary kinematic (5 GLMs)", spss: "trunk_ratio, shoulder_vert_norm, elbow_angle_mean, movement_time_sec, peak_velocity_px_s; Holm–Bonferroni k=5" },
...
const LEGACY_KIN_MAP = {
  ...
  elbow_angle_mean: ["elbow_angle_mean", "elbow_angle_max", "total_max_elbow_deg", "elbow_extension"],
  movement_time_sec: ["movement_time_sec", "total_duration_s", "duration"],
  peak_velocity_px_s: ["peak_velocity_px_s", "peak_velocity_m_s", "total_peak_velocity"],
};

export function pickKinField(result, canonicalKey) {
  if (!result || typeof result !== "object") return null;
  if (canonicalKey === "elbow_angle_mean" && result.elbow_angle_reliable === false) return null;
  const aliases = LEGACY_KIN_MAP[canonicalKey] || [canonicalKey];
...
export function normalizeKinematicResult(result) {
  ...
  KINEMATIC_VARS.forEach(({ key }) => {
    const v = pickKinField(result, key);
    if (v !== null) out[key] = v;
  });
...
export function kinCrossPhaseComparable(kinematicsResults, metricKey, armForPhase = null) {
  ...
  if (metricKey === "elbow_angle_mean") {
    if (!phases.every((p) => kinematicsResults[p]?.elbow_angle_reliable === true)) return false;
    const allWorld = phases.every((p) => kinematicsResults[p]?.elbow_angle_method === "mediapipe_world");
    if (!allWorld && armForPhase) {
      const arms = new Set(phases.map((p) => armForPhase(kinematicsResults, p)).filter(Boolean));
      if (arms.size > 1) return false;
    }
    return true;
  }
...
function appendManuscriptAliases(row, suffix, m) {
  ...
  const legacy = {
    ...
    total_peak_velocity: kinCell(m, "peak_velocity_px_s"),
  };
...
export function generateStudySPSSSyntax(csvFilename = "master_study_data.csv") {
  ...
  l("  delta_sparc delta_trunk_ratio delta_shoulder_vert_norm delta_elbow_angle_mean delta_movement_time_sec delta_peak_velocity_px_s");
