...
def tremor_peak_freq_hz(
    ...
    if peak_power <= 1e-12:
        return float("nan")
    if require_snr and len(band_spec) >= 3:
        median = float(np.median(band_spec))
        if peak_power < TREMOR_PEAK_SNR * max(median, 1e-12):
            return float("nan")
...
def tremor_noise_floor_px(shoulder_width: Optional[float] = None) -> float:
    try:
        sw = float(shoulder_width) if shoulder_width is not None else 0.0
    except (TypeError, ValueError):
        sw = 0.0
    if not np.isfinite(sw) or sw <= 0:
        sw = 0.0
    return float(max(TREMOR_NOISE_FLOOR_PX, 0.004 * sw))


def _bandpass_zero_phase(y: np.ndarray, fs: float, f_lo: float = 8.0, f_hi: float = 12.0) -> np.ndarray:
    ...
    nyq = fs * 0.5
    hi = min(float(f_hi), nyq * 0.45)
    lo = max(float(f_lo), 1.0)
    if not (hi > lo):
        return np.zeros_like(y)
    try:
        b, a = butter(2, [lo / nyq, hi / nyq], btype="band")
        pad = min(3 * max(len(a), len(b)), max(1, len(y) - 1))
        return filtfilt(b, a, y, padlen=pad)
    except Exception:
        return np.zeros_like(y)


def tremor_band_abs_rms_px(
    palm_x: np.ndarray,
    palm_y: np.ndarray,
    fs: float,
    start: int,
    ...
    """RMS of 8–12 Hz palm displacement in video pixels."""
    if end <= start or fs <= 0:
        return float("nan")
    px = np.asarray(palm_x[start : end + 1], dtype=float)
    py = np.asarray(palm_y[start : end + 1], dtype=float)
    if len(px) < max(16, int(round(min_duration_s * fs))):
        return float("nan")
    px = _linear_detrend(px)
    py = _linear_detrend(py)
    bx = _bandpass_zero_phase(px, fs)
    by = _bandpass_zero_phase(py, fs)
    pad = min(max(4, int(round(fs * 0.12))), max(1, len(px) // 5))
    mag2 = bx * bx + by * by
    if pad * 2 < len(mag2):
        mag2 = mag2[pad : len(mag2) - pad]
    finite = np.isfinite(mag2)
    if int(np.sum(finite)) < 8:
        return float("nan")
    return float(np.sqrt(np.mean(mag2[finite])))
...
def compute_tremor_metrics_block(
    *,
    ...
    out: Dict[str, Any] = {}
    abs_rms = tremor_band_abs_rms_px(palm_x, palm_y, fs, start, end)
    floor = tremor_noise_floor_px(sw if np.isfinite(sw) else None)
    present = bool(np.isfinite(abs_rms) and abs_rms >= floor)
    if np.isfinite(abs_rms):
        out["tremor_abs_rms_px"] = round(float(abs_rms), 3)
        if np.isfinite(sw) and sw > 0:
            out["tremor_abs_rms_sw"] = round(float(abs_rms) / sw, 6)
    out["tremor_present"] = present
    if not present:
        out["hand_speed_tremor_8_12hz_power"] = 0.0
        out["tremor_8_12hz_power"] = 0.0
        out["tremor_index"] = 100.0
        return out
    ...
    hand_peak = tremor_peak_freq_hz(np.asarray(palm_x[start : end + 1], dtype=float), fs)
