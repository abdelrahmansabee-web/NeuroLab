...
def _pause_and_stops(
    speed: np.ndarray,
    fs: float,
    start: int,
    end: int,
    pause_threshold_frac: float = 0.05,
    min_pause_frames: Optional[int] = None,
    exclude_terminal_dwell: bool = True,
    terminal_tail_frac: float = 0.18,
) -> Tuple[float, int]:
    """
    Path pauses/stops inside [start, end].

    Terminal (end-of-window) low-speed dwell — typical grasp fixation or mouth hold —
    is excluded from the primary counts when exclude_terminal_dwell=True so that a
    completed grasp does not inflate pause time relative to an incomplete reach.
    """
    split = _pause_and_stops_split(
        speed,
        fs,
        start,
        end,
        pause_threshold_frac=pause_threshold_frac,
        min_pause_frames=min_pause_frames,
        exclude_terminal_dwell=exclude_terminal_dwell,
        terminal_tail_frac=terminal_tail_frac,
    )
    return float(split["pause_time_sec"]), int(split["number_of_stops"])


def _pause_and_stops_split(
    speed: np.ndarray,
    fs: float,
    start: int,
    end: int,
    ...
    """Split path pauses vs terminal grasp/mouth dwell."""
    speed = np.asarray(speed, dtype=float)
    n = len(speed)
    start = int(max(0, min(start, max(0, n - 1))))
    end = int(max(start, min(end, max(0, n - 1))))
    seg = speed[start : end + 1]
    if len(seg) == 0 or fs <= 0:
        return {
            "pause_time_sec": 0.0,
            "number_of_stops": 0,
            "pause_time_sec_total": 0.0,
            "number_of_stops_total": 0,
            "grasp_dwell_sec": 0.0,
            "grasp_dwell_stops": 0,
            "terminal_dwell_excluded": False,
        }

    if min_pause_frames is None:
        min_pause_frames = max(3, int(round(0.08 * fs)))

    peak = float(np.nanmax(seg)) if np.any(np.isfinite(seg)) else 0.0
    thr = pause_threshold_frac * peak if peak > 0 else 1.0
    below = np.asarray(seg < thr, dtype=bool)

    runs: List[Tuple[int, int]] = []  # inclusive indices relative to seg
    i = 0
    while i < len(below):
        if not below[i]:
            i += 1
        ...
        while j + 1 < len(below) and below[j + 1]:
            j += 1
        if (j - i + 1) >= min_pause_frames:
            runs.append((i, j))
        i = j + 1

    win_len = max(1, end - start + 1)
    terminal_cut = int(np.floor(win_len * (1.0 - float(np.clip(terminal_tail_frac, 0.05, 0.45)))))
    # A run is terminal dwell if it ends at the window end, or starts in the final tail.
    def _is_terminal(run: Tuple[int, int]) -> bool:
        r0, r1 = run
        touches_end = r1 >= (len(seg) - 1)
        in_tail = r0 >= terminal_cut
        return bool(touches_end or in_tail)

    path_runs = []
    dwell_runs = []
    for run in runs:
        if exclude_terminal_dwell and _is_terminal(run):
            dwell_runs.append(run)
        else:
            path_runs.append(run)

    # If every pause was classified as terminal (very short incomplete reach that
    # ends in a pause), keep them as path pauses — otherwise incomplete Pre would
    # look artificially pause-free.
    if exclude_terminal_dwell and runs and not path_runs and dwell_runs:
        path_runs = list(dwell_runs)
        dwell_runs = []

    def _sum_frames(rs: List[Tuple[int, int]]) -> int:
        return int(sum(r1 - r0 + 1 for r0, r1 in rs))

    path_frames = _sum_frames(path_runs)
    dwell_frames = _sum_frames(dwell_runs)
    total_frames = path_frames + dwell_frames
    return {
        "pause_time_sec": path_frames / fs,
        "number_of_stops": len(path_runs),
        "pause_time_sec_total": total_frames / fs,
        "number_of_stops_total": len(runs),
        "grasp_dwell_sec": dwell_frames / fs,
        "grasp_dwell_stops": len(dwell_runs),
        "terminal_dwell_excluded": bool(exclude_terminal_dwell and dwell_frames > 0),
    }
...
def build_movement_profile(
    *,
    ...
    pause_split = _pause_and_stops_split(spd_full, fs, start, end)
    pause_time_sec = float(pause_split["pause_time_sec"])
    number_of_stops = int(pause_split["number_of_stops"])
    pause_time_sec_total = float(pause_split["pause_time_sec_total"])
    number_of_stops_total = int(pause_split["number_of_stops_total"])
    grasp_dwell_sec = float(pause_split["grasp_dwell_sec"])
    grasp_dwell_stops = int(pause_split["grasp_dwell_stops"])
    ...
    profile: Dict[str, Any] = {
        ...
        "pause_time_sec_total": round(pause_time_sec_total, 3),
        "number_of_stops_total": int(number_of_stops_total),
        "grasp_dwell_sec": round(grasp_dwell_sec, 3),
        "grasp_dwell_stops": int(grasp_dwell_stops),
        "terminal_dwell_excluded": bool(pause_split.get("terminal_dwell_excluded")),
...
def merge_profile_into_results(results: Dict[str, Any], profile: Dict[str, Any]) -> None:
    ...
    mapping = {
        ...
        "grasp_dwell_stops": "grasp_dwell_stops",
        "terminal_dwell_excluded": "terminal_dwell_excluded",
