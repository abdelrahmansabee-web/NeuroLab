...
def _resolve_affected_side_with_meta(df: pd.DataFrame, affected_side: str) -> Tuple[str, Dict[str, Any]]:
    """Resolve analyzed arm — explicit override, CSV metadata, or kinematic fusion."""
    side = (affected_side or "auto").lower()
    if side in ("left", "right"):
        return side, {"method": "explicit", "confidence": 1.0}

    if "affected_side" in df.columns:
        s = str(df["affected_side"].iloc[0]).lower()
        if s in ("left", "right"):
            meta: Dict[str, Any] = {"method": "csv_column", "confidence": 1.0}
            if "side_detection_confidence" in df.columns:
                try:
                    meta["confidence"] = float(df["side_detection_confidence"].iloc[0])
                except (TypeError, ValueError):
                    pass
            if "side_detection_scores" in df.columns:
                try:
                    import json

                    meta["scores"] = json.loads(str(df["side_detection_scores"].iloc[0]))
                except Exception:
                    pass
            return s, meta

    raw_landmarks = (
        "LEFT_WRIST_X" in df.columns
        or "left_wrist_x" in df.columns
        or "LEFT_WRIST_x" in df.columns
    )
    if raw_landmarks:
        fw = int(df["frame_width_px"].iloc[0]) if "frame_width_px" in df.columns else DEFAULT_FRAME_WIDTH
        ...
        det = detect_active_arm_with_meta(df, fw, fh, fs=fs)
        return det["side"], {
    ...
    return "right", {"method": "fallback", "confidence": 0.0}


def _resolve_affected_side(df: pd.DataFrame, affected_side: str) -> str:
    side, _meta = _resolve_affected_side_with_meta(df, affected_side)
    return side
...
def _landmarks_from_mediapipe_csv(
    ...
    *,
    ...
    pinky_x = _col(work, f"{p}_PINKY", "X") * frame_width
    pinky_y = _col(work, f"{p}_PINKY", "Y") * frame_height
    try:
        thumb_x = _col(work, f"{p}_THUMB", "X") * frame_width
        thumb_y = _col(work, f"{p}_THUMB", "Y") * frame_height
    except KeyError:
        thumb_x = np.full(len(wrist_x), np.nan)
        thumb_y = np.full(len(wrist_x), np.nan)
    ...
    if side.lower() == "right":
        opp_shoulder_x, opp_shoulder_y = ls_x, ls_y
    else:
        opp_shoulder_x, opp_shoulder_y = rs_x, rs_y
...
def analyze_trial(
    ...
    # Lower-extremity templates (STS / squat / gait / balance) — separate analyzer.
    try:
        from le_gait_analysis import is_le_clinical_task, analyze_le_task

        if is_le_clinical_task(clinical_task):
            return analyze_le_task(
                native_df,
                clinical_task,
                fs=native_fs,
        ...
        return {"error": f"LE analysis failed: {le_err}"}

    resolved_view = _resolve_camera_view(native_df, camera_view)
    resolved_side, side_detection_meta = _resolve_affected_side_with_meta(native_df, affected_side)
    ...
    index_x = coords.get("index_x")
    index_y = coords.get("index_y")
    pinky_x = coords.get("pinky_x")
    pinky_y = coords.get("pinky_y")
    thumb_x = coords.get("thumb_x")
    thumb_y = coords.get("thumb_y")
    opp_shoulder_x = coords.get("opp_shoulder_x")
    opp_shoulder_y = coords.get("opp_shoulder_y")
    ...
    if raw_df is not None:
        ...
        hand3d = load_hand_landmarks_3d(raw_df, side, frame_width, frame_height)
        arm_world = load_arm_world(raw_df, side)
        hand_world = load_hand_world(raw_df, side)
        if arm3d and upsampled:
            arm3d = {
                k: _upsample_array(v, native_fs, fs) if isinstance(v, np.ndarray) else v
                for k, v in arm3d.items()
            }
        if hand3d and upsampled:
            hand3d = {
                k: _upsample_array(v, native_fs, fs) if isinstance(v, np.ndarray) else v
                for k, v in hand3d.items()
            }
        if arm_world and upsampled:
            arm_world = {k: _upsample_array(v, native_fs, fs) for k, v in arm_world.items()}
        if hand_world and upsampled:
            hand_world = {k: _upsample_array(v, native_fs, fs) for k, v in hand_world.items()}
    ...
    results: Dict[str, Any] = {
        ...
        "side_analyzed": side,
        "side_detection_method": side_detection_meta.get("method"),
        "side_detection_confidence": side_detection_meta.get("confidence"),
        "side_detection_scores": side_detection_meta.get("scores"),
        "analysis_frame": "body_normalized_world" if arm_world else ("body_normalized_3d" if arm3d else "body_normalized_2d"),
    ...
    clinical_task_key = (clinical_task or "study_reach_grasp").strip().lower()
    if clinical_task_key not in (
        "study_reach_grasp",
        ...
        clinical_task_key = "study_reach_grasp"
    ...
    if reach_fwd:
        ...
        results["hand_displacement_method"] = f"table_{int(TABLE_WIDTH_CM)}cm_forward_outbound"
    elif reach_lit:
        ...
        results["hand_displacement_method"] = f"table_{int(TABLE_WIDTH_CM)}cm_trunk_peak_fallback"
    else:
    ...
    # Physical elevation via clinic table scale (85 cm width → cm/px).
    if cm_per_px and cm_per_px > 0 and np.isfinite(shoulder_elev_abs):
        results["shoulder_elevation_cm"] = float(shoulder_elev_abs * cm_per_px)
    else:
        results["shoulder_elevation_cm"] = float("nan")
    ...
    if cm_per_px and cm_per_px > 0 and np.isfinite(peak_vel):
        ...
        results["peak_velocity_scale"] = f"table_{int(TABLE_WIDTH_CM)}cm"
    else:
        results["peak_velocity_cm_s"] = float("nan")
        results["peak_velocity_scale"] = None
    results["time_to_peak_velocity_sec"] = ttpv
    results["relative_time_to_peak_pct"] = rel_ttpv
    if np.isfinite(results.get("shoulder_elevation_cm", float("nan"))):
        results["shoulder_elevation_scale"] = f"table_{int(TABLE_WIDTH_CM)}cm"
    else:
        results["shoulder_elevation_scale"] = None
    ...
    from movement_profile import build_movement_profile, merge_profile_into_results
    ...
    profile = build_movement_profile(
        ...
        arm3d=arm3d,
        trunk_ratio=float(trunk_path_ratio) if np.isfinite(trunk_path_ratio) else float("nan"),
        trunk_cheat_ratio=float(results.get("trunk_cheat_ratio") or float("nan")),
        velocity_threshold=velocity_threshold,
        opp_shoulder_x=opp_shoulder_x,
        opp_shoulder_y=opp_shoulder_y,
        index_x=index_x,
        ...
        affected_side=side,
        clinical_task=clinical_task_key,
    )
    merge_profile_into_results(results, profile)

    from adl_task_phases import analyze_clinical_task_phases

    adl = analyze_clinical_task_phases(
        ...
        arm3d=arm3d,
        trunk_ratio=float(trunk_path_ratio) if np.isfinite(trunk_path_ratio) else float("nan"),
        trunk_cheat=float(results.get("trunk_cheat_ratio") or float("nan")),
        velocity_threshold=velocity_threshold,
        primary_reach_start=kin_start,
        primary_reach_end=kin_end,
        opp_shoulder_x=opp_shoulder_x,
        opp_shoulder_y=opp_shoulder_y,
        index_x=index_x,
        index_y=index_y,
    ...
    results["clinical_task"] = adl.get("clinical_task")
    results["clinical_task_label"] = adl.get("clinical_task_label")
    results["task_phases"] = adl.get("task_phases") or []
    results["movement_bouts_detected"] = adl.get("movement_bouts_detected")
    results["task_complete"] = adl.get("task_complete")
    results["task_completion_ratio"] = adl.get("task_completion_ratio")
    results["expected_phase_ids"] = adl.get("expected_phase_ids")
    results["completed_phase_ids"] = adl.get("completed_phase_ids")
    results["nvp_reach"] = adl.get("nvp_reach")
    results["nvp_transport"] = adl.get("nvp_transport")
    results["nvp_drink"] = adl.get("nvp_drink")
    results["nvp_return"] = adl.get("nvp_return")
    results["nvp_total"] = adl.get("nvp_total")
    results["lift_height_sw"] = adl.get("lift_height_sw")
    results["lift_height_px"] = adl.get("lift_height_px")
    results["drink_lift_height_sw"] = adl.get("drink_lift_height_sw")
    # Physical lift height via clinic table scale (85 cm width → cm/px).
    lift_px = adl.get("lift_height_px")
    lift_cm = px_to_cm(float(lift_px), cm_per_px) if lift_px is not None else float("nan")
    if np.isfinite(lift_cm):
        results["lift_height_cm"] = round(float(lift_cm), 2)
        if adl.get("drink_lift_height_sw") is not None:
            results["drink_lift_height_cm"] = round(float(lift_cm), 2)
    else:
        results["lift_height_cm"] = None
        results["drink_lift_height_cm"] = None
    results["straightness_reach"] = adl.get("straightness_reach")
    results["pause_time_sec_reach"] = adl.get("pause_time_sec_reach")
    results["number_of_stops_reach"] = adl.get("number_of_stops_reach")
    results["grasp_dwell_sec"] = adl.get("grasp_dwell_sec")
    results["grasp_dwell_stops"] = adl.get("grasp_dwell_stops")
    results["functional_hold_sec"] = adl.get("functional_hold_sec")
    results["sip_bout_count"] = adl.get("sip_bout_count")
    results["nvp_excludes_sip_bouts"] = bool(adl.get("nvp_excludes_sip_bouts", True))
    results["primary_smoothness_window"] = adl.get("primary_smoothness_window") or "reach_grasp"
    # Primary study NVP = reach window (extra sips do not count). Phase totals are separate.
    if adl.get("nvp_reach") is not None:
        if results.get("nvp_full_task") is None and results.get("nvp") is not None:
            results["nvp_full_task"] = results.get("nvp")
        results["nvp"] = adl.get("nvp_reach")
    if adl.get("nvp_total") is not None and results.get("nvp_full_task") is None:
        results["nvp_full_task"] = adl.get("nvp_total")
    if adl.get("straightness_reach") is not None:
        if results.get("straightness_full_task") is None and results.get("straightness") is not None:
            results["straightness_full_task"] = results.get("straightness")
        results["straightness"] = adl.get("straightness_reach")
    if adl.get("pause_time_sec_path") is not None:
        # Prefer path-only pause (excludes grasp dwell + transport mouth hold).
        results["pause_time_sec"] = adl.get("pause_time_sec_path")
        results["number_of_stops"] = adl.get("number_of_stops_path")
        results["pause_time_sec_path"] = adl.get("pause_time_sec_path")
        results["number_of_stops_path"] = adl.get("number_of_stops_path")
    elif adl.get("pause_time_sec_reach") is not None:
        if results.get("pause_time_sec_total") is None and results.get("pause_time_sec") is not None:
            results["pause_time_sec_total"] = results.get("pause_time_sec")
        results["pause_time_sec"] = adl.get("pause_time_sec_reach")
        results["number_of_stops"] = adl.get("number_of_stops_reach")
    # Also surface totals from primary movement profile when present
    mp = results.get("movement_profile") or {}
    if results.get("pause_time_sec_total") is None and mp.get("pause_time_sec_total") is not None:
        results["pause_time_sec_total"] = mp.get("pause_time_sec_total")
        results["number_of_stops_total"] = mp.get("number_of_stops_total")
    if results.get("grasp_dwell_sec") is None and mp.get("grasp_dwell_sec") is not None:
        results["grasp_dwell_sec"] = mp.get("grasp_dwell_sec")
        results["grasp_dwell_stops"] = mp.get("grasp_dwell_stops")
    if results.get("nvp_full_task") is None and mp.get("nvp") is not None and adl.get("nvp_reach") is not None:
        # Profile nvp may still be full-window; keep as exploratory full-task.
        if mp.get("nvp") != adl.get("nvp_reach"):
            results["nvp_full_task"] = mp.get("nvp")
    results["reach_window_start_frame"] = adl.get("reach_window_start_frame")
    results["reach_window_end_frame"] = adl.get("reach_window_end_frame")
...
def analyze_stroke_kinematic_csv(
    ...
    landmark_side = raw.get("side_analyzed")
    if landmark_side not in ("left", "right"):
        landmark_side = _resolve_affected_side(df, affected_side)
    sw_px = None
    if "palm_x" in df.columns and "shoulder_width" in df.columns:
        ...
        try:
            _, _, sw_px_val = _landmarks_from_mediapipe_csv(
                df, affected_side=landmark_side, frame_width=frame_width, frame_height=frame_height
            )
    ...
    result: Dict[str, Any] = {
        ...
        "side_detection_method": raw.get("side_detection_method"),
        "side_detection_confidence": raw.get("side_detection_confidence"),
        "side_detection_scores": raw.get("side_detection_scores"),
        ...
        "shoulder_elevation_cm": _r(raw.get("shoulder_elevation_cm"), 2),
        "shoulder_elevation_scale": raw.get("shoulder_elevation_scale"),
        ...
        "peak_velocity_scale": raw.get("peak_velocity_scale"),
        ...
        "nvp": raw.get("nvp"),
        "nvp_full_task": raw.get("nvp_full_task"),
        "nvp_reach": raw.get("nvp_reach"),
        "nvp_transport": raw.get("nvp_transport"),
        "nvp_drink": raw.get("nvp_drink"),
        "nvp_return": raw.get("nvp_return"),
        "nvp_total": raw.get("nvp_total"),
        "lift_height_sw": _r(raw.get("lift_height_sw"), 4),
        "lift_height_px": _r(raw.get("lift_height_px"), 2),
        "lift_height_cm": _r(raw.get("lift_height_cm"), 2),
        "drink_lift_height_sw": _r(raw.get("drink_lift_height_sw"), 4),
        "drink_lift_height_cm": _r(raw.get("drink_lift_height_cm"), 2),
        "sip_bout_count": raw.get("sip_bout_count"),
        "nvp_excludes_sip_bouts": raw.get("nvp_excludes_sip_bouts"),
        "primary_smoothness_window": raw.get("primary_smoothness_window"),
        "straightness": _r(raw.get("straightness"), 4),
        "straightness_full_task": _r(raw.get("straightness_full_task"), 4),
        "straightness_reach": _r(raw.get("straightness_reach"), 4),
        "pause_time_sec": _r(raw.get("pause_time_sec"), 3),
        "number_of_stops": raw.get("number_of_stops"),
        "pause_time_sec_total": _r(raw.get("pause_time_sec_total"), 3),
        "number_of_stops_total": raw.get("number_of_stops_total"),
        "pause_time_sec_path": _r(raw.get("pause_time_sec_path"), 3),
        "number_of_stops_path": raw.get("number_of_stops_path"),
        "pause_time_sec_reach": _r(raw.get("pause_time_sec_reach"), 3),
        "number_of_stops_reach": raw.get("number_of_stops_reach"),
        "grasp_dwell_sec": _r(raw.get("grasp_dwell_sec"), 3),
        "grasp_dwell_stops": raw.get("grasp_dwell_stops"),
        "functional_hold_sec": _r(raw.get("functional_hold_sec"), 3),
        "terminal_dwell_excluded": raw.get("terminal_dwell_excluded"),
        "elbow_angle_mean_deg": _r(raw.get("elbow_angle_mean_deg"), 1),
        "elbow_angle_max_deg": _r(raw.get("elbow_angle_max_deg"), 1),
        "elbow_angle_range_deg": _r(raw.get("elbow_angle_range_deg"), 1),
        "peak_elbow_ang_vel_deg_s": _r(raw.get("peak_elbow_ang_vel_deg_s"), 1),
        "shoulder_elevation_palm_ratio": _r(raw.get("shoulder_elevation_palm_ratio"), 4),
        "movement_quality_index": _r(raw.get("movement_quality_index"), 1),
        "compensation_index": _r(raw.get("compensation_index"), 3),
        "task_pattern": raw.get("task_pattern"),
        "movement_profile": raw.get("movement_profile"),
        "clinical_task": raw.get("clinical_task"),
        "clinical_task_label": raw.get("clinical_task_label"),
        "task_phases": raw.get("task_phases"),
        "movement_bouts_detected": raw.get("movement_bouts_detected"),
        "task_complete": raw.get("task_complete"),
        "task_completion_ratio": raw.get("task_completion_ratio"),
        "expected_phase_ids": raw.get("expected_phase_ids"),
        "completed_phase_ids": raw.get("completed_phase_ids"),
        "nvp_reach": raw.get("nvp_reach"),
        "straightness_reach": _r(raw.get("straightness_reach"), 4),
        "pause_time_sec_reach": _r(raw.get("pause_time_sec_reach"), 3),
        "number_of_stops_reach": raw.get("number_of_stops_reach"),
    ...
    if metric_scale and metric_scale > 0 and sw_px and sw_px > 0:
        ...
        if result.get("peak_velocity_px_s") is not None:
            result["peak_velocity_m_s"] = round(result["peak_velocity_px_s"] * px_to_m, 3)
        # Prefer clinic table scale for shoulder_elevation_cm; shoulder-width scale is fallback only.
        if result.get("shoulder_elevation_cm") is None and result.get("shoulder_elevation_abs_px") is not None:
            result["shoulder_elevation_cm"] = round(result["shoulder_elevation_abs_px"] * px_to_m * 100, 2)
            result["shoulder_elevation_scale"] = "shoulder_width_metric_scale"
