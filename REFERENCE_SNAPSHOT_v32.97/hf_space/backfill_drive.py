...
def rebuild_clinic_folder(data_dir: Path) -> Dict[str, Any]:
    ...
    from drive_persist import list_clinic_folder, reconcile_drive_to_program, reorganize_clinic_folder
    ...
    try:
        reconciled = reconcile_drive_to_program(program_patients)
    except Exception as exc:
        reconciled = {"ok": False, "error": str(exc)[:300]}
    try:
    ...
    return {
        "ok": bool(reconciled.get("ok") or reorganized.get("ok") or filled.get("ok")),
