...
export function ValidationOverlayPlayer({
  ...
  const drawOverlay = useCallback(() => {
    ...
    const jointDots = [];
    const inMovementWin = idx >= win.start_idx && idx <= win.end_idx;
    const fingerActive = inMovementWin && idx > win.start_idx;
    const hlAnchorNorm = pt("hl_wrist") || pt("wrist");
    const maxFingerNorm = 0.38;
    const nearHand = (cpt) => {
      if (!cpt || !hlAnchorNorm) return false;
      return Math.hypot(cpt[0] / cw - hlAnchorNorm[0], cpt[1] / ch - hlAnchorNorm[1]) <= maxFingerNorm;
    };

    if (fingerActive && fingerJoints) {
      HAND_FINGER_ORDER.forEach((fid) => {
        const fj = fingerJoints[fid];
        if (!fj) return;
        JOINT_ORDER.forEach((jname) => {
          const visOk = fj.vis ? fj.vis[jname] === true : false;
          if (!visOk) return;
          const fjNext = fNext?.finger_joints?.[fid];
          const cpt = jointToCanvas(fj[jname], fjNext?.[jname]);
          if (!cpt || !nearHand(cpt)) return;
          jointDots.push({ fid, jname, cpt, style: JOINT_DOT[jname] });
        });
      });
    }
    ...
    if (jointDots.length > 0) {
        ...
        HAND_FINGER_ORDER.forEach((fid) => {
          ...
          for (let i = 0; i < chain.length - 1; i += 1) {
            drawChalkStick(ctx, chain[i], chain[i + 1], {
              ...
              width: 1.65,
              blur: shadowOff != null ? shadowOff : 4,
              curve: 0.06,
            });
          }
        });
      ...
      ctx.globalAlpha = useHandHl && Number.isFinite(fingerTrackConf) && fingerTrackConf > 0
        ? Math.min(1, Math.max(0.72, fingerTrackConf))
        : 1;
      jointDots.forEach(({ cpt, style }) => {
        if (overlayStyle === "chalk") {
          drawChalkJoint(ctx, cpt, { r: style.r, dim: style.jname !== "tip" });
        } else {
          dot(cpt, { fill: style.fill, stroke: style.stroke, r: style.r });
        }
      });
      ctx.restore();
