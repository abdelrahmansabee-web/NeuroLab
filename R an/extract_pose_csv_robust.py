...
import numpy as np
import pandas as pd
from pose_xyz_fill import fill_landmark_xyz_keep_visibility
...
def interpolate_landmarks(df: pd.DataFrame, max_gap: int = 8) -> pd.DataFrame:
    """Fill XYZ for Analyze. Do not copy later visibility onto unseen PRE rest."""
    return fill_landmark_xyz_keep_visibility(df, max_gap=max_gap)
