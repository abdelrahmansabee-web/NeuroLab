...
(function () {
  ...
  function uploadPatients(awaitResult) {
    try {
      var dump = {};
      ...
      if (awaitResult) {
        return fetch("/api/ipad-localstorage", req).then(function (res) {
          return res.json().catch(function () {
            return {};
          }).then(function (body) {
            if (!res.ok) throw new Error(body.detail || "HTTP " + res.status);
  ...
  function bindMenu() {
    ...
    window.__nlForceIpadPatientsUpload = function () {
      return uploadPatients(true);
    };
